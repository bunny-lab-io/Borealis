...
def test_runtime_paths_ignore_stale_override_when_running_tree_is_known(tmp_path, monkeypatch) -> None:
    ...
    (repo_root / "Agent.exe").parent.mkdir(parents=True, exist_ok=True)
    (repo_root / "Agent.exe").write_text("", encoding="utf-8")
...
def test_runtime_paths_resolve_installed_agent_tree_before_nested_agent_markers(tmp_path) -> None:
    install_root = tmp_path / "C" / "Borealis"
    script_path = install_root / "Agent" / "Borealis" / "Roles" / "role_system_wireguard.py"
    script_path.parent.mkdir(parents=True, exist_ok=True)
    script_path.write_text("# test", encoding="utf-8")
    (install_root / "Agent" / "Agent.exe").write_text("", encoding="utf-8")

    assert runtime_paths.find_project_root(script_path) == install_root
    assert runtime_paths.agent_runtime_root(script_path) == install_root / "Agent"
    assert runtime_paths.agent_borealis_root(script_path) == install_root / "Agent" / "Borealis"
