...
def _find_borealis_root() -> Optional[str]:
    ...
    discovered = None
    for _ in range(8):
        if os.path.isfile(os.path.join(cur, 'Agent.exe')) or os.path.isfile(os.path.join(cur, 'Agent.ps1')):
            discovered = cur
            break
    ...
    if override:
        ...
        if os.path.isfile(os.path.join(candidate, 'Agent.exe')) or os.path.isfile(os.path.join(candidate, 'Agent.ps1')):
            try:
    ...
    if os.path.isfile(os.path.join(fallback, 'Agent.exe')) or os.path.isfile(os.path.join(fallback, 'Agent.ps1')):
        return fallback
    return None
