...
func hasRuntimeFlag(args []string) bool {
	for _, arg := range args {
		switch arg {
		case "--service", "--helper", "--config-path", "--install-service", "--uninstall-service", "--update-check", "--watchdog-check", "--finalize-update", "--validate-config", "--update-metadata", "--field", "--value", "--once", "--version":
			return true
		}
	}
	return false
