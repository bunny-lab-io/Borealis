...
func preparePayloadSource(cfg BootstrapConfig, logger *BootstrapLogger) (string, func(), error) {
	...
	if fileExists(cfg.PayloadPath) {
		...
		if !sourceRootHasGoAgent(sourceRoot) {
			return "", nil, fmt.Errorf("agent payload missing Go Agent source under Data\\Agent")
		}
	...
	sourceRoot := resolveSourceRoot(extractRoot)
	if !sourceRootHasGoAgent(sourceRoot) {
		return "", nil, fmt.Errorf("downloaded source archive missing Go Agent source under Data\\Agent")
	}
...
func sourceRootHasGoAgent(root string) bool {
	return fileExists(filepath.Join(root, "Data", "Agent", "go.mod")) ||
		fileExists(filepath.Join(root, "Data", "Agent", "cmd", "agent", "main.go")) ||
		fileExists(filepath.Join(root, "Data", "Agent", "Agent.exe")) ||
		fileExists(filepath.Join(root, "Data", "Agent", "dist", "windows-amd64", "Agent.exe")) ||
		fileExists(filepath.Join(root, "Agent.exe")) ||
		fileExists(filepath.Join(root, "go.mod")) && fileExists(filepath.Join(root, "cmd", "agent", "main.go"))
}
