...
class AgentKeyStore:
    ...
    def build_ssl_context(self) -> Optional[ssl.SSLContext]:
        ...
        include_default = self._should_layer_default_trust(certs)
        trusted_first = self._should_enable_trusted_first(certs, include_default)
        try:
            setattr(context, "_borealis_layered_default", include_default)
        except Exception:
            pass
        try:
            setattr(context, "_borealis_trusted_first", trusted_first)
        except Exception:
            pass
        ...
        if verify_flag is not None and trusted_first:
            try:
    ...
    def _should_enable_trusted_first(
        self, certs: List["x509.Certificate"], include_default: bool
    ) -> bool:
        if include_default:
            return True
        ...
        for cert in certs:
            try:
                basic = cert.extensions.get_extension_for_class(x509.BasicConstraints)  # type: ignore[attr-defined]
            except Exception:
                continue

            if getattr(basic.value, "ca", False):
                return True
