...
func TestSourceReceiptRejectsPrivateAmbiguousAndReplayedResults(t *testing.T) {
	...
	for name, bad := range map[string][]byte{
		"unknown":        bytes.Replace(receipt, []byte(`"version":2`), []byte(`"version":2,"private":"do-not-publish"`), 1),
		...
		"duplicate":      bytes.Replace(receipt, []byte(`"version":2`), []byte(`"version":3,"version":2`), 1),
		...
		"missing":        bytes.Replace(receipt, []byte(`"version":2,`), nil, 1),
		"null":           bytes.Replace(receipt, []byte(`"version":2`), []byte(`"version":null`), 1),
		"version":        bytes.Replace(receipt, []byte(`"version":2`), []byte(`"version":1`), 1),
