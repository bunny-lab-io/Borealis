...
def _discover_official_catalog_root(config: Optional[Mapping[str, object]] = None) -> Path:
    ...
    for candidate in (module_path, *module_path.parents):
        data_dir = candidate / "Official_Assemblies"
        if data_dir.is_dir():
            return data_dir.resolve()
        data_dir = candidate / "Data" / "Engine" / "Containers" / "api-backend" / "data" / "Official_Assemblies"
        if data_dir.is_dir():
            return data_dir.resolve()
    ...
    for candidate in _path_discovery_roots(module_path):
        ...
        try:
            ...
            continue
    raise RuntimeError("Could not locate bundled official assemblies directory (expected Data/Engine/Containers/api-backend/data/Official_Assemblies).")
