...
func registerSoftwareRoutes(mux *http.ServeMux, auth *authService, fallback http.Handler) {
	...
	mux.HandleFunc("POST /api/device/software/{hostname}/uninstall", deviceSoftwareUninstallHandler(auth))
	...
	mux.HandleFunc("POST /api/software/uninstall", bulkSoftwareUninstallHandler(auth))
