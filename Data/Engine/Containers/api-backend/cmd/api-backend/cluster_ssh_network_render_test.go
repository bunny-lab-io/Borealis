...
func TestClusterSSHNetworkRenderLivePeerVIPComposition(t *testing.T) {
	for _, mode := range []string{"expansion", "replacement", "VIP changed", "target changed", "credential lost", "cancel", "cached", "zero", "wrong result", "changed input", "key alias", "probe error"} {
		t.Run(mode, func(t *testing.T) {
			...
			var cached clusterremote.TargetNetworkRender
			probes := 0
			probe := func(ctx context.Context, item clusterSSHInspectedTarget, request clusterremote.NetworkRenderRequest, check func(context.Context) error) (clusterremote.TargetNetworkRender, error) {
				probes++
				if mode == "cached" && probes > 1 {
					return cached, nil
				}
				if mode == "zero" {
					return clusterremote.TargetNetworkRender{}, nil
				}
				if mode == "probe error" {
					return clusterremote.TargetNetworkRender{}, errors.New("private SSH")
				}
				if mode == "key alias" {
				...
				var value clusterremote.TargetNetworkRender
				err := sshNetworkFixtureObserve(t, ctx, item, keys[item.Binding.TargetID], resultLink, request.Targets.Peers, nil, func(ssh *clusterremote.Client) error {
					var err error
					value, err = ssh.InspectNetworkRender(ctx, []byte("fixture-sudo"), clusterremote.Target{Address: item.Binding.Address, Port: item.Binding.Port}, item.Key, request, check)
					return err
				})
				if mode == "VIP changed" {
			...
			err := client.observeNetworkRender(ctx, authority, source, target, probe, refresh, a.Lease, a.Baseline, r.sealed())
