...
func analyzeEngineBackupPayload(payload engineBackupPayload) map[string]any {
	summary := []engineBackupAnalysisRow{
		...
		{ID: "filters", Name: "Device Filters", Count: engineBackupTableRowCount(payload, "engine.device_filters")},
		{ID: "metadata_fields", Name: "Metadata Fields", Count: engineBackupTableRowCount(payload, "engine.metadata_field_definitions")},
		{ID: "software_inventory", Name: "Software Inventory", Count: engineBackupTableRowCount(payload, "engine.device_software_inventory")},
		{ID: "patch_inventory", Name: "Patch Inventory", Count: engineBackupTableRowCount(payload, "engine.device_patch_inventory")},
...
func engineBackupTableSpecs() []engineBackupTableSpec {
	return []engineBackupTableSpec{
		...
		{Name: "engine.device_list_views", OrderBy: []string{"id"}, Export: false, Restore: false, ResetSerials: true},
		...
		{Name: "engine.device_patch_inventory", OrderBy: []string{"device_guid", "patch_key", "id"}, Export: true, Restore: true, ResetSerials: true},
		{Name: "engine.patch_catalog_entries", OrderBy: []string{"kb", "patch_key", "id"}, Export: true, Restore: true, ResetSerials: true},
		{Name: "engine.patch_policies", OrderBy: []string{"id"}, Export: true, Restore: true, ResetSerials: true},
		{Name: "engine.patch_policy_sites", OrderBy: []string{"policy_id", "site_id"}, Export: true, Restore: true, ResetSerials: true},
		{Name: "engine.patch_policy_targets", OrderBy: []string{"policy_id", "id"}, Export: true, Restore: true, ResetSerials: true},
		{Name: "engine.patch_policy_exclusions", OrderBy: []string{"policy_id", "id"}, Export: true, Restore: true, ResetSerials: true},
		{Name: "engine.patch_policy_rules", OrderBy: []string{"policy_id", "id"}, Export: true, Restore: true, ResetSerials: true},
...
func engineBackupFileSpecs() []engineBackupFileSpec {
	return []engineBackupFileSpec{
		...
		{ID: "traefik_local_ca_cert", Path: func() (string, error) { return overviewLocalCACertPath(), nil }, Mode: 0o644},
