...
- For container deployment changes, run shell syntax checks for `bootstrap.sh`, `Engine.sh`, `Agent.sh`, `Borealis.sh`, and `Update.sh`, then validate `Data/Engine/Containers/compose.yaml` through `docker compose -f Data/Engine/Containers/compose.yaml config`.
