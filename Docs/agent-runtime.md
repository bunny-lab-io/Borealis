...
- Linux runtime copy lives in `Agent/` and is regenerated by `Agent.sh deploy` (or `bootstrap.sh -Agent` -> `Agent.sh deploy`).
- Windows runtime copy is regenerated by `Borealis.ps1` (or `bootstrap.ps1` -> `Borealis.ps1`).
...
- Linux bootstrap: `bootstrap.sh -Agent` installs dependencies, syncs source, then runs `Agent.sh deploy`.
- Local Linux redeploy: `Agent.sh deploy`; this never updates git.
