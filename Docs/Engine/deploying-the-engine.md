...
During deployment, Borealis installs missing dependencies, reconciles the single-node K3s baseline, prepares runtime configuration, builds changed service container images, applies K3s bridge workloads, and starts the Docker Compose stack automatically.
...
`webui-frontend` and `remote-desktop-guacd` workloads are also reconciled into K3s as single-replica ClusterIP-only Deployments. Production WebUI traffic is routed by the existing Compose Traefik edge to the K3s `webui-frontend` ClusterIP after that Service is ready. Compose Traefik still owns HTTP/HTTPS, ACME or local CA certificate state, and watched dynamic route files under `Engine/Services/traefik-edge/config/dynamic/`. Compose API and Compose guacd remain authoritative until later cutover stages. In production, `BOREALIS_WEBUI_TRAFFIC_OWNER=k3s` disables the Compose WebUI container after K3s routing is active; rollback recreates it by setting `BOREALIS_WEBUI_TRAFFIC_OWNER=docker-compose` before redeploy. Dev-mode WebUI keeps the existing Compose/HMR route by default unless `BOREALIS_WEBUI_TRAFFIC_OWNER=k3s` is explicitly set. Scoped WebUI and guacd rebuilds also refresh the K3s workloads so they follow the current image manifest before the next full deploy.
...
??? example "Detailed Codex Breakdown"
    ...
    - Stage 4 deploys fixed-template `webui-frontend` and `remote-desktop-guacd` bridge workloads into K3s with ClusterIP Services only. `Engine/Deploy/k3s-bridge-workloads.sha256` records bridge manifest inputs. Scoped `webui-frontend` and `remote-desktop-guacd` rebuilds reconcile the K3s baseline, operator, and workload manifests after the Compose service refresh.
    - Stage 6 production WebUI cutover keeps Compose Traefik as the only public edge and dynamic route-file reader, then changes its core WebUI upstream to the K3s `webui-frontend` ClusterIP by writing `BOREALIS_WEBUI_TRAFFIC_OWNER=k3s` and `BOREALIS_WEBUI_UPSTREAM_HOST=<cluster-ip>` into `Engine/Deploy/compose.env`. Dev mode defaults to `docker-compose` traffic owner so HMR stays on the existing path. Production `k3s` ownership removes the Compose WebUI container from the active Compose service set; rollback is `BOREALIS_WEBUI_TRAFFIC_OWNER=docker-compose Engine.sh --network-mode public|local deploy prod`.
