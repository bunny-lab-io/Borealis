...
!!! info "System Requirements"
    ...
    **DNS Records & Certificate Considerations**:
    ...
    - Public TCP `443` must reach Borealis Traefik end-to-end with TLS and ALPN unchanged. An outer reverse proxy must use TCP/TLS pass-through for Borealis hostname; it must not terminate Borealis TLS. Borealis uses Let's Encrypt TLS-ALPN-01 so upstream HTTP challenge routing is not required.
...
Verified installation keeps `/opt/Borealis` Git metadata and source writable by operator account that invoked `sudo`. Engine restores that ownership after successful deployment because root-run Git identity checks can refresh index metadata. Root-owned Engine and Agent runtime state stays excluded from ownership change, so development `git checkout` and `git pull` commands need no `sudo`.
...
Multi-node mode is separate, gated conversion. Current release supports one or three homogeneous AMD64 Ubuntu nodes, per-node application workloads, one Cluster Virtual IP, CloudNativePG, paired quorum admission, existing HMR isolation recovery, and one-node-at-time release updates. API and WebUI fence odd-numbered expansion or shrinking beyond three nodes; that work remains future roadmap scope. Read [Managing Engine Clusters](managing-engine-clusters.md) before attempting conversion. If Engine FQDN uses outer or nested reverse proxy, include [Cluster Virtual IP upstream cutover](managing-engine-clusters.md#cut-over-the-outer-reverse-proxy) in conversion plan. Production conversion requires explicit operator checkpoint after stable-K3s conformance and disposable three-node qualification.
...
??? example "Detailed Codex Breakdown"
    ...
    - Every sudo-origin launch validates `SUDO_USER`, `SUDO_UID`, and `SUDO_GID` against local account records after optional Git checkout. It transfers ownership of checkout root, `.git`, and non-runtime source to that operator before command dispatch, then restores ownership after successful dispatch in case root-run Git inspection refreshed index metadata. `Engine`, `Engine.old`, and `Agent` remain pruned; root automation without validated sudo-origin identity does not change ownership.
    ...
    - Fresh Longhorn apply first waits initial manifest workloads, then waits for controller-created `daemonset/longhorn-csi-plugin` before applying required 200-second liveness guard. Second rollout inventory catches CSI controller Deployments and DaemonSets created after initial manifest snapshot before storage-class reconciliation continues.
    ...
    - Stage 10 made K3s `wireguard-tunnel` the tunnel traffic owner. Its control process creates or validates group-readable server keys, atomically renders base config, activates `borealis-wg`, installs peer route and restrictive firewall chains, and opens UDP `30000` before readiness can pass. Engine keeps only WireGuard `config/` and `secrets/` directories owner/group-writable at `0770` so root control process running with Borealis group can create files without `DAC_OVERRIDE`; contained key/config files remain `0640`. API starts afterward and reuses same identity for peer reconciliation. Lifecycle withdrawal persists a short reactivation fence until Pod exits; explicit reconcile clears that fence and repairs owner runtime. Failed rollout automatically appends Pod and host diagnostics to `Engine/Deploy/build.log`. Later deploy detects retained `ProgressDeadlineExceeded`, restarts unchanged failed Deployment, then performs normal bounded rollout wait instead of failing immediately on stale status.
    - Stage 11 made K3s `traefik-edge` the public edge owner. It runs one host-network Deployment, preserves HTTP/HTTPS ports, ACME/local CA state, and watched dynamic route files, and removes stale Compose Traefik plus site-worker-orchestrator containers during deploy. Public ACME uses TLS-ALPN-01 on TCP `443`; standalone deployment verifies hostname directly against local Traefik and refuses to report WebUI readiness while only default self-signed certificate is available.
    ...
    - The central SSH capacity gate uses fresh CPU/RAM from every original target and requires the source's explicit effective PostgreSQL cap, preserving service-limit precedence without inventing release defaults. It rechecks the entire source configuration around observation and consumption. Its bounded profile/cap check does not replace `load_profile_tuning`'s computed-profile guard, aggregate workload/storage qualification or preparation authorization. [Cluster guidance](managing-engine-clusters.md) owns this gate's lifecycle and supported quantity contract.
    - `Tests/Unit_Tests/test_engine_cluster_recovery.py` exercises actual Bash library functions for all profiles, larger targets, capped memory, unchanged standalone detection, missing/malformed/insufficient contracts, inherited prejoin configuration and subsequent host telemetry changes. Go sizing tests independently compare all profile boundaries and effective quantity conversion with isolated pure functions extracted from `Engine.sh`; they do not execute its startup or deployment path. No lab runtime is modified.
