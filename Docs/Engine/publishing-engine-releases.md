...
??? example "Detailed Codex Breakdown"
    ...
    - The S01 receiver must independently verify published immutable GitHub release/channel/tag/source identity, both asset digests, bounded manifest/download and exact inner identity before extraction or execution. The packaging tool does not attest publication or authorize a host mutation. Receiver/SSH preparation integration remains under [S01](https://github.com/bunny-lab-io/Borealis/issues/521); archive availability does not enable the current connection-check dialog to join nodes.
