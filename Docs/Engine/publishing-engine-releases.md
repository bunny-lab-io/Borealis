...
Before publication, wait for packaging to pass. Inspect bootstrap archive/manifest, `borealis-node-images-linux-amd64.json`, all nine `borealis-node-image-<service>-linux-amd64.oci.tar` assets and the four K3s assets listed below. Verify GitHub asset digests and matching release/source identities. Packaging requires Docker Buildx and builds production images once in an isolated builder. K3s binary and bundled images use the release's pinned baseline. These assets are intended for delivery by the existing Engine; operators do not download or execute them on joining hosts. Packaging alone does not qualify SSH enrollment.
...
- `borealis-node-k3s-linux-amd64` binary
- `borealis-node-k3s-install.sh` pinned installer input (not automatically executed)
- `borealis-node-k3s-images-linux-amd64.tar` bundled K3s images
- `borealis-node-k3s-linux-amd64.json` input identity and measured image demand
...
??? example "Detailed Codex Breakdown"
    ...
    - `Tests/tools/build_node_k3s_assets.py` uses its own exact-tag checkout, requires `release-manifest.json` to match the embedded `internal/clusterbootstrap/k3s_inputs.lock.json`, and downloads only the pinned public K3s binary, uncompressed airgap archive and version-matched installer script. Installer bytes come from the pinned version source with an independent checksum; mutable `get.k3s.io` content is not a packaging input. Exact sizes/SHA256 values come from reviewed source, not current upstream metadata. Only one HTTPS GitHub release-asset redirect is allowed; requests carry no credentials and failures suppress signed URLs. The source-built helper scans the archive before writing the inventory; output is never overwritten and failed packaging removes only its newly created output. K3s is never executed or imported. Workflow uploads all four inputs to the unpublished draft alongside application assets.
    - `k3s_inputs.go` hashes the complete pinned archive and every content blob, checks the complete eight-image set and Linux/AMD64 configs, then measures each gzip layer against its DiffID. Repeated layers receive no expansion/entry discount. Exact archive authentication also binds the upstream OCI indexes and their selected platform. This parser is specific to the source-pinned K3s archive, not a general multi-platform importer. Shared layer validation accepts explicit `./` prefixes, canonicalizes them before duplicate checks and still rejects traversal, special/sparse members and concatenated gzip. Baseline upgrades must update reviewed input pins and prove the new archive; changing only a version string cannot pass.
    - K3s inventory version2 additionally binds the embedded runtime payload. The reviewed binary lock records its exact zstd frame offset/size/hash and expanded tar hash/bytes/file bytes/entry/CNI-link counts. Packaging verifies the complete binary first, copies only that authenticated frame into private scratch, decodes with `zstd` under memory/output/time bounds, hashes all expanded bytes and scans tar entries without extraction or execution. Links stay inert and must resolve within the payload root. Failed/expired decoder processes are killed and joined. This avoids accidentally reusing an installed K3s runtime: the upstream launcher may fall back to its default data directory even when a different data directory was requested. Frame identity and extraction behavior follow the pinned [packaging script](https://github.com/k3s-io/k3s/blob/v1.36.3%2Bk3s1/scripts/package-cli) and [launcher](https://github.com/k3s-io/k3s/blob/v1.36.3%2Bk3s1/cmd/k3s/main.go). Any binary/payload change requires reviewed pins and remeasurement, not a guessed expansion ratio.
    - `cluster_ssh_k3s.go` resolves the four immutable Borealis release assets, compares binary/archive identities with compiled pins and reads only the small inventory during qualification. Fresh source baseline, publication identities and all target claims bracket capacity observation. K3s demands include incoming inputs, two retained binary/archive generations, content blobs, measured expanded image entries and two embedded-runtime generations with additional data/current/previous/lock/CNI entries; paths sharing a filesystem share one budget. The inventory remains conditional: installer-generated host files, OS packages, external operators, runtime growth and remaining metadata reserves are not yet complete. K3s bulk transfer, journaled binary installation and native import/admission remain gated; existing application image staging alone does not deliver these inputs. The eventual fixed installer follows K3s's [offline installation contract](https://docs.k3s.io/installation/airgap), with these independently pinned installer bytes and existing controller authority. Merely packaging the script does not authorize executing it or starting/enabling K3s.
