...
??? example "Detailed Codex Breakdown"
    ...
    - `cluster_ssh_worker_runtime.go` wires an Aegis-capable API consumer for only `ssh_onboarding` operations at `inspect_ssh_targets`. It polls every10seconds, considers at most two targets, and claims each through the existing database lease. Locked replicas do not dispatch. Replica races yield one owner per target; shutdown cancels and joins workers before Aegis listeners and database close. The atomic queue store and Admin submission/progress APIs are implemented. The full cohort wizard is not wired yet; the existing connection-check dialog still performs only its original checks.
