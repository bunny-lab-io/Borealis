...
??? example "Detailed Codex Breakdown"
    ...
    - `cluster_ssh_preparation.go` composes freshly verified bootstrap assets with `internal/clusterbootstrap/preparation_config.go` and `preparation.go`. This is a preparation-input component, not enabled remote preparation. The source observer, guarded controller transition, fixed journal-backed writer, identity delivery and joining remain unfinished. Existing target-claim SQL rejects the reserved `prepare_ssh_targets` / `stage_source` phase; an inspection claim cannot authorize it. The receiver still accepts only `verify`.
