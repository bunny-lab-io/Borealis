...
??? example "Detailed Codex Breakdown"
    ...
    - `cluster_ssh_preparation.go` composes freshly verified bootstrap assets with `internal/clusterbootstrap/preparation_config.go` and `preparation.go`. This is a preparation-input component, not enabled remote preparation. Source observation primitives are implemented below; the guarded controller transition, source-action transport, fixed journal-backed writer, identity delivery and joining remain unfinished. Existing target-claim SQL rejects the reserved `prepare_ssh_targets` / `stage_source` phase; an inspection claim cannot authorize it. The receiver still accepts only `verify`.
    ...
    - `prepareClusterSSHTargetFromSource` now assembles this production client instead of requiring an impossible worker/controller capability combination. Its existing preparation-input factory repeats the same broker read during immutable bundle verification/export. Production still cannot claim preparation phases: guarded qualification/confirmation and a live worker lease guard across the whole download/export/remote session remain required before activation. Broker success proves source acquisition only; it does not authorize identity installation, host preparation, membership changes or public admission.
