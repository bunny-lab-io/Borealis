...
??? example "Detailed Codex Breakdown"
    ...
    - `ExecutorIdentity` exposes public unit/invocation/boot/cgroup identity for future journal integration. It is not proof that an older executor stopped. Before mutation or absence reconciliation, the fixed dispatcher must durably bind that identity to the mutation intent, retain the host-wide journal lock and independently prove prior executors quiescent. Managed services intentionally created by a future install/join step need their own domain-state proof; executor containment cannot stand in for membership fencing or service rollback.
    ...
    - The exclusive nonblocking host lock stays held throughout synchronous mutation and proof callbacks. `Apply` supplies a duplicate lock descriptor for every subprocess `ExtraFiles`; a surviving child then retains the same lock after parent loss. Callbacks must honor context and cancel/join all descendants before returning. Descriptor inheritance alone cannot prove a process group or cgroup has stopped: descendants may close inherited descriptors. The supervised receiver supplies process containment, while durable executor identity and independent quiescence proof must still be wired into the fixed dispatcher before takeover or absence reconciliation.
    - Private canonical JSON binds the immutable repository/release/source SHA, cluster/operation/target, approved SSH identity, actual machine/address, operation attempt and owner generation. Reopening rejects changed source/stable identity, older attempts/generations and a different holder or attempt at the same generation. New ownership preserves each step's producing binding. Fixed lock/journal files must be regular, receiver-owned mode0600 with one hardlink; symlinks, FIFOs, unsafe modes, oversized or ambiguous JSON fail closed. Newly created files receive exact private permissions despite restrictive umask; existing unsafe files are never repaired implicitly.
