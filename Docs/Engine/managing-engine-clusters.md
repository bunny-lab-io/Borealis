...
Ubuntu 24.04 or newer on AMD64 is required. A detected or unknown Borealis/K3s installation requires further inspection before preparation. Filesystem figures describe the filesystem containing `/opt`, falling back to the root filesystem when `/opt` is unavailable; they do not verify Longhorn storage or its replica capacity.
