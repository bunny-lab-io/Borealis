...
??? example "Detailed Codex Breakdown"
    ...
    - Cluster controller renews its 20-second PostgreSQL ownership lease every five seconds through an independent heartbeat while operation steps run. Explicit ownership loss cancels step context immediately. Transient database errors during primary-service handoff are retried for at most 15 seconds; persistent failure cancels before lease expiry. Claim, advance, failure, and completion transactions lock and verify same live lease row before changing durable operation state, so former holder cannot record progress after fencing.
    - Controller ownership acquisition uses a five-second database deadline. A standby holding a stale TCP connection to the former CloudNativePG primary therefore returns to its two-second claim loop instead of hanging until kernel TCP timeout after primary loss.
