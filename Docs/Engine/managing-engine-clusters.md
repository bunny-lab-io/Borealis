...
??? example "Detailed Codex Breakdown"
    ...
    - `withNetworkOwner` composes the live broker inside completed source/target peer acquisition. The collector passes owned copies of both peers and source network inputs; combined short checks retain both authorities, and full checks reobserve native pinned SSH peers followed by actual source VIP ownership before and after consumption. Failed combined checks latch cancellation even if ignored. Full rechecks must fit the remaining 30-second VIP scope; slow environments fail closed and require measured optimization, not relaxed freshness. Production per-target claim/credential assembly, packet exchanges, durable qualification, confirmation and mutation remain disabled/open work.
