...
??? example "Detailed Codex Breakdown"
    ...
    - `cluster_ssh_worker_runtime.go` wires an Aegis-capable API consumer for only `ssh_onboarding` operations at `inspect_ssh_targets`. It polls every10seconds, considers at most two targets, and claims each through the existing database lease. Locked replicas do not dispatch. Replica races yield one owner per target; shutdown cancels and joins workers before Aegis listeners and database close. The parent onboarding operation builder/queue is not implemented yet, so existing operations cannot activate this consumer.
