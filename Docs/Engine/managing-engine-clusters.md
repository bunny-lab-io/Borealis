...
??? example "Detailed Codex Breakdown"
    ...
    - Durable S01 internals: `cluster_ssh_credentials.go` encrypts exact operation/target/SSH-identity-bound envelopes only while Aegis is configured, unlocked and its active key verifies current database generation. `cluster_ssh_store.go` prepares two-hour credential records and database-clock target leases with monotonically increasing owner generation. Claim/renew/load require the sole active operation and exact target step; old owners, expired credentials and Aegis changes fail closed. Existing eligible controller performs bounded startup/periodic cleanup, deleting credentials and fencing workers without rewriting target outcome state. [Database reference](../Reference/Data%20and%20Schema/db-reference.md) owns table/lifecycle details. Queue/start/resubmission and mutating worker steps remain unfinished; the connection-check dialog does not yet join nodes.

    - `cluster_ssh_worker_runtime.go` wires an Aegis-capable API consumer for only `ssh_onboarding` operations at `inspect_ssh_targets`. It polls every10seconds, considers at most two targets, and claims each through the existing database lease. Locked replicas do not dispatch. Replica races yield one owner per target; shutdown cancels and joins workers before Aegis listeners and database close. The atomic queue store is implemented; public submission and wizard integration are not wired yet, so current operator actions cannot activate this consumer.
    ...
    - These are internal store contracts. No new public route or WebUI field is introduced here; Admin submission, request idempotency/recovery and wizard integration remain unfinished. Queue tests use actual isolated PostgreSQL with paired/replacement admission shapes, concurrent requests, source/Aegis changes and retained reservations. Progress tests prove pending and historical evidence behavior and exclude synthetic private storage from output. No live target was prepared or joined by this component.
