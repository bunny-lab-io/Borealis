...
??? example "Detailed Codex Breakdown"
    ...
    - This is controller-local observation plumbing with portable fixtures. Connecting it to the Kubernetes-blind API worker requires a live scoped broker protocol that preserves ongoing Lease and all-source checks; serializing a controller timestamp or adding an old owner receipt to a snapshot is insufficient. Whole-cohort target claim/credential ownership, bounded ARP, remaining qualification/preparation and public confirmation stay gated. No deployment, production worker activation, new Kubernetes permission, dependency or database change is included.
