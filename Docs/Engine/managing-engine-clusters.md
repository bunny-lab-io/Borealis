...
??? example "Detailed Codex Breakdown"
    ...
    - The observer reads `/v1-k3s/config`, its own Node and `/version`, then rereads supervisor configuration/version and local machine/boot identity. `internal/clusterbootstrap/source_network.go` selects actual global pod/service ranges and checks singular/plural agreement, canonical IPv4 masks, wholly RFC1918 nonoverlapping ranges, default service DNS address, cluster domain, VXLAN/agent networking, standard supervisor ports and supported Borealis component/encryption settings. Component argument overrides and unsupported critical settings fail; extending inheritance requires extending this contract. The Node allocation corroborates membership within the global pod range and never supplies that global range. Actual server version must equal Node kubelet version; Node UID/name, machine/boot IDs, Ready and unique InternalIP must agree.
    - Upstream protocol reference: pinned [K3s Control fields](https://github.com/k3s-io/k3s/blob/v1.36.3%2Bk3s1/pkg/daemons/config/types.go) and [supervisor handler](https://github.com/k3s-io/k3s/blob/v1.36.3%2Bk3s1/pkg/server/handlers/handlers.go). The full response contains unrelated private fields and host paths. Only public `SourceNetwork` projection can leave the observer: canonical nonzero UUID Node/boot IDs (36characters), lowercase Linux hostname (63ASCII characters), nonzero lowercase machine ID (32hex characters), K3s version (32ASCII characters) and canonical private IPv4 pod/service CIDRs. No frontend text entry or public Engine API is added.
    ...
    - `source-network-client` is a separate fixed node-manager command. It accepts only the public nonce and downward-API Job/Pod UIDs, reads the fixed local manager token/socket, and returns static errors. A strict manager response parser permits only successful `InspectSourceNetwork` and its public `SourceNetwork` projection. It writes a version1 receipt to the kubelet-provided regular `/dev/termination-log` file, without following symlinks or creating a replacement. No private settings, credentials, raw supervisor fields or manager errors enter stdout or Pod status.
    - `internal/clusterbootstrap/source_receipt.go` limits receipts to2048UTF-8 bytes. Internal nonce/Job/Pod IDs are nonzero canonical UUIDs (36characters); version is exactly1; network fields use the existing public source field classes/limits. Reject missing/unknown/duplicate/case-aliased keys at every selected level, bad encoding, trailing JSON, truncation, mismatched IDs or invalid networks. No frontend text entry or public Engine route is added. Controller independently compares Node UID/name/machine/boot/version against current cohort; a serialized receipt alone grants no authority.
