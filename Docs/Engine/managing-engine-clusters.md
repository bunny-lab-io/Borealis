...
??? example "Detailed Codex Breakdown"
    ...
    - Runtime activation remains separate: no production phase calls the preparation factory yet, and this source change is not a live deployment. The future guarded broker must connect Aegis-capable worker authority to controller-owned reads while maintaining the exact worker lease throughout acquisition/export. Qualification/confirmation, identity delivery, fixed journaled writes and original-controller admission stay gated. Portable Job/Pod fixtures do not qualify published-image execution on a live node.

    ### Preparation authority database adapter

    - `cluster_ssh_preparation_authority.go` binds one reader to the exact target lease, immutable expected repository/release/SHA and sealed credential envelope already held by its Aegis-capable worker. `prepareClusterSSHTargetFromSource` constructs this adapter directly. Each read has a10second bound and verifies the matching configured Aegis generation plus actual unlocked memory key before and after database observation. The store never decrypts while holding a transaction. Static errors exclude payloads, ciphertext, generation tokens and database diagnostics.
