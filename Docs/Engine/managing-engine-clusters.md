...
??? example "Detailed Codex Breakdown"
    ...
    - Cluster-enable and node-redeploy retries pass recorded immutable baseline SHA through controller Job and fixed node-manager contract. Node manager gives only Engine child process command-scoped Git safe-directory trust, and workload initialization resolves same pinned commit without mutating global Git configuration. Later operator checkout movement cannot change retry revision.
