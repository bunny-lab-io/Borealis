...
Next Step: Re-test the refreshed Engine + Agent build on a weaker host and confirm the new single-shot `vnc_connect_retry` plus `vnc_backend_connect` confirmation fully eliminate repeated shared-listener churn during successful sessions.
