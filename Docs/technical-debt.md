...
Current Mitigation: `Data/Engine/services/API/devices/software_uninstall.py` now loads `Data/Engine/services/API/devices/software_uninstall_blocklist.json`, matches Fedora Media Writer's known-bad quiet metadata, and returns an unsupported uninstall capability with a clear operator-visible reason rather than dispatching the command to the agent.
...
Files: `Data/Engine/services/API/devices/software_uninstall.py`, `Data/Engine/services/API/devices/software_uninstall_blocklist.json`, `Data/Engine/Unit_Tests/test_devices_api.py`
...
Files: `Data/Agent/Roles/role_system_software_management.py`, `Data/Engine/services/API/devices/software_uninstall.py`, `Data/Engine/services/API/devices/services.py`, `Data/Engine/web-interface/src/Devices/Tabs/Installed_Software.jsx`
...
Impact: The new release-channel system can stage and apply most source changes immediately, but top-level updater wrapper-script fixes may still lag because the currently running wrapper is preserved until the next update pass.
Root Cause: The agent now applies full-repo zip artifacts in place while the PowerShell/shell wrapper scripts are still executing, so overwriting those files mid-run risks corrupting the active update process.
...
Evidence: The staged zip-apply path now uses `_SYNC_EXCLUDED_RELATIVE = {"Update.ps1", "Update.sh"}` to avoid overwriting the actively executing wrapper scripts during `prepare-update`, while still allowing `Data/Agent/update_helper.py` and `Data/Agent/update_state.py` to update as a matched pair.
...
Files: `Data/Engine/services/API/devices/vnc.py`, `Data/Engine/services/RemoteDesktop/vnc_proxy.py`, `Data/Engine/services/VPN/vpn_tunnel_service.py`, `Data/Agent/Roles/role_system_vnc.py`
...
Files: `Data/Engine/services/API/devices/routes.py`, `Data/Engine/services/API/devices/management.py`, `Data/Agent/agent.py`, `Data/Agent/Roles/role_system_device_auditor.py`
...
Current Mitigation: `role_system_device_auditor.py` writes system identity fields to summary and mirrors them into `cpu` (`system_model`, `system_serial_number`, etc.); `Device_Summary.jsx` reads those keys as fallback.
...
Files: `Data/Agent/Roles/role_system_device_auditor.py`, `Data/Engine/web-interface/src/Devices/Tabs/Device_Summary.jsx`
