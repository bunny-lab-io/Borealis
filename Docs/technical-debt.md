...
Current Mitigation: `Data/Engine/services/API/devices/vnc.py` re-emits `vpn_tunnel_start` with `vnc_bootstrap`, `Data/Engine/services/RemoteDesktop/vnc_proxy.py` escalates stalled backend connects with `vnc_connect_retry`, `Data/Engine/services/VPN/vpn_tunnel_service.py` throttles repeated listener recovery, the shell bridge keeps quiet shell sessions off the same stale-transport path with idle keepalives, and `Data/Agent/Roles/role_VNC.py` now suppresses the periodic firewall-success log spam so the agent-side `vnc.log` stays readable while the transport issue is still open.
