...
- `agent.json` stores `schema_version`, `server_url`, `enrollment_code`, `agent.guid`, `agent.agent_id`, `agent.branch`, `agent.installed_build_id`, `agent.log_retention_days`, `agent.state`, `agent.liveness`, `agent.dependency_state`, `agent.metadata_fields`, Ed25519 keys, access/refresh tokens, and Engine script-signing trust material.
- `agent.metadata_fields` is sparse and keyed as `field_001` through `field_500`. Each populated field stores `value`, `modified_at`, `source`, and optional `actor`; empty fields are removed after Engine acknowledgement.
...
- Heartbeats include `metadata_fields` from `agent.json` when present. Engine responses can return `metadata_fields` updates when Engine has the newest value and `metadata_field_acks` when a blank clear has been accepted.
...
- Metadata fields can be updated by scripts through `Agent.exe --update-metadata --field=1 --value="text"` on Windows or `Agent --update-metadata --field=1 --value="text"` on Linux. Blank values queue clears; values are plain text and capped at 1024 characters.
