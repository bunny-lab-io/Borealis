...
- If a source-channel update finds the configured branch deleted on GitHub, Windows and Linux updaters rewrite the local target to `source`/`main`, restart the Agent when needed so runtime heartbeats report `main`, and continue from the main branch instead of failing the update check.
