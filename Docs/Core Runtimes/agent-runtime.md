...
- `agent.json` stores `schema_version`, `server_url`, `enrollment_code`, `agent.guid`, `agent.agent_id`, `agent.branch`, `agent.installed_build_id`, `agent.log_retention_days`, `agent.state`, `agent.liveness`, `agent.dependency_state`, `dependency_versions.wireguard`, `dependency_versions.ultravnc`, Ed25519 keys, access/refresh tokens, and Engine script-signing trust material.
