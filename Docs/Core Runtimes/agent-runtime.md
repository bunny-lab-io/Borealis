...
- `agent.metadata_fields` is sparse and keyed as `field_001` through `field_500`. Each populated field stores base64-encoded `value`, `modified_at`, `source`, and optional `actor`; empty fields are removed after Engine acknowledgement.
