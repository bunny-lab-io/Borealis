...
- Windows targets use the same IP/FQDN scope model. Borealis tries SMB `ADMIN$` plus Remote Service Control Manager, then remote scheduled task, then WMI/DCOM process creation, then WinRM. Windows onboarding stages Go `Agent.exe`, which owns install, repair, update checks, support dependency setup, `config.json`, scheduled-task registration, and runtime. When `Agent.exe` validates an already-running enrolled agent, the onboarding target is marked skipped with `Already Enrolled and Active` instead of completed. If policy blocks all methods, Borealis records that manual agent installation is required.
