...
- Windows targets use the same IP/FQDN scope model. Borealis tries SMB `ADMIN$` plus Remote Service Control Manager, then remote scheduled task, then WMI/DCOM process creation, then WinRM. Windows onboarding installs from `C:\Borealis`, preserves `C:\Borealis\Agent`, uses one host-wide onboarding mutex, stops `python.exe` / `pythonw.exe` with `Stop-Process` plus `taskkill.exe` fallback before updating, cleans stale per-run onboarding source folders/processes plus stopped `BorealisOnboarding*` service/task remnants, and skips repeat attempts for a matching pending approval for 24 hours. If policy blocks all methods, Borealis records that manual agent installation is required.
