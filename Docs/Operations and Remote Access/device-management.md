...
- Windows targets use the same IP/FQDN scope model. Borealis tries SMB `ADMIN$` plus Remote Service Control Manager, then remote scheduled task, then WMI/DCOM process creation, then WinRM. Windows onboarding stages `Agent_Service_Bootstrapper.exe`, which owns remote launch mechanics before starting `Agent.ps1`. When that bootstrapper validates an already-running enrolled agent, the onboarding target is marked skipped with `Already Enrolled and Active` instead of completed. `Agent.ps1` installs from `C:\Borealis`, preserves `C:\Borealis\Agent`, `C:\Borealis\Temp`, and `C:\Borealis\Dependencies`, and skips repeat attempts for a matching pending approval. If policy blocks all methods, Borealis records that manual agent installation is required.
