...
- Windows targets use the same IP/FQDN scope model. Borealis tries SMB `ADMIN$` plus Remote Service Control Manager, then remote scheduled task, then WMI/DCOM process creation, then WinRM. If policy blocks all methods, Borealis records that manual agent installation is required.
