...
- Agent bootstrap/update diagnostics: `<AgentInstallRoot>/Logs/bootstrap.log`. Windows bootstrap truncates this file at each start so it contains only the latest run, and always writes verbose trace/command output there. Operator-facing console/GUI output stays limited to high-level steps, warnings, and errors.
...
- Cross-platform updater traces are written to `<AgentInstallRoot>/Logs/bootstrap.log`; Windows bootstrap starts by truncating this file and keeps verbose output out of operator-facing stdout/stderr streams.
