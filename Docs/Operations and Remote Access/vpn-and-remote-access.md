...
- The Windows VNC role mirrors `ultravnc.ini` beside the active `winvnc.exe` and points the service `binPath` at that mirrored file, because UltraVNC service startup is sensitive to config locality after runtime staging.
