...
- The Windows VNC role computes UltraVNC-compatible VNC DES password hashes internally when the UltraVNC ZIP does not include a password helper, including the UltraVNC checksum byte expected in `ultravnc.ini`, so missing `createpassword.exe` must not leave `BorealisAgentUltraVNC` stopped.
