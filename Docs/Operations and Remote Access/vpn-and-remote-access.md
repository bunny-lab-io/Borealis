...
- Before issuing a Guacamole token, the Engine can complete an RFB VNCAuth challenge and ServerInit probe against the advertised UltraVNC listener. If authentication fails, the Engine emits `vnc_refresh` with `reason=vnc_auth_retry`; the Agent rotates the runtime VNC credential and reloads UltraVNC once, while the Engine waits through slower Windows SCM stop/start cycles before probing the refreshed listener again. The Engine returns `vnc_backend_auth_failed` if the refreshed listener still rejects the advertised password.
