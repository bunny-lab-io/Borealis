...
- Before issuing a Guacamole token, the Engine can complete an RFB VNCAuth challenge and ServerInit probe against the advertised UltraVNC listener. If authentication fails, the Engine emits `vnc_refresh` with `reason=vnc_auth_retry`; the Agent rotates the runtime VNC credential and the Engine probes the refreshed listener again. If UltraVNC reports too many authentication attempts, the Engine returns `vnc_backend_auth_rate_limited`, caches that agent lockout window, and skips further RFB probes until the retry window expires so repeated browser clicks do not extend the lockout. The Engine returns `vnc_backend_auth_failed` if the refreshed listener still rejects the advertised password.
