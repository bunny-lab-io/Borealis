...
- The Windows VNC role computes UltraVNC-compatible VNC DES password hashes internally when the UltraVNC ZIP does not include a password helper, so missing `createpassword.exe` must not leave `BorealisAgentUltraVNC` stopped.
