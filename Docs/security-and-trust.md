...
- Operator MFA methods: Borealis supports TOTP authenticator apps and WebAuthn passkeys for browser sign-in.
...
- Borealis operator accounts still start with username/password, but MFA setup and verification can now be satisfied with either a TOTP authenticator app or a WebAuthn passkey when the browser and server support it.
...
- `POST /api/auth/mfa/reset` (Token Authenticated) - clear the current operator's MFA setup, including authenticator secrets and passkeys, so MFA setup is required on the next login.
- `POST /api/auth/passkeys/register/options` (Authenticated or MFA pending) - start a passkey registration ceremony.
- `POST /api/auth/passkeys/register/verify` (Authenticated or MFA pending) - verify a passkey registration response and store the credential.
- `POST /api/auth/passkeys/authenticate/options` (Token Authenticated, MFA pending) - start a passkey sign-in ceremony.
- `POST /api/auth/passkeys/authenticate/verify` (Token Authenticated, MFA pending) - verify a passkey sign-in response and complete login.
