...
- Before the proxy connects, the Engine prewarms the cached WireGuard path for registered agents, then fast-probes the agent's advertised UltraVNC listener; it only re-emits VNC startup with `reason=vnc_bootstrap` when that probe misses and the backend likely still needs a refresh.
