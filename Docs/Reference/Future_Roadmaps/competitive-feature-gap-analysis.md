...
| WUA monitoring / remediation | Partial but still a gap | `Docs/Using the Platform/patch-management.md`, `Docs/Reference/Data and Schema/api-reference.md`, `Docs/Using the Platform/device-auditing.md` | Borealis can audit pending WUA items, installed WUA history, and schedule WUA installs through normal job history, but does not enforce Windows Update policy. |
...
| LDAP/LDAPS directory authentication | Shipped | `Data/Engine/Containers/api-backend/cmd/api-backend/directory_management.go`, `Data/Engine/Containers/api-backend/cmd/api-backend/directory_ldap.go`, `Data/Engine/Containers/webui-frontend/data/web-interface/src/Access_Management/Directory_Services.jsx`, `Data/Engine/Containers/site-worker/data/database.py` | Borealis now supports directory credential providers with LDAPS certificate trust, provider testing, AD group role mapping, and site-scoped operator assignment by directory group. |
...
- Current Borealis position:
  ...
  - Borealis now ships Windows patch inventory, WUA-style pending/update-history audit, and scheduled ad-hoc install triggers.
...
- LDAP/LDAPS directory authentication with AD group role and site assignment:
  ...
  - `Data/Engine/Containers/site-worker/data/database.py`
