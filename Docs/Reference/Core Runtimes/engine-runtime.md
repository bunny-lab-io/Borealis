...
??? example "Detailed Codex Breakdown"
    ...
    - The Engine requests the current Agent VNC password on demand over the registered Agent Socket.IO channel during `/api/vnc/establish`, uses that live credential for the Guacamole token it is minting, and does not maintain an agent-level VNC password cache. Normal launches require a non-auth RFB banner before returning browser bootstrap data, so a TCP-only or hung UltraVNC listener fails fast as `vnc_backend_no_rfb_banner` instead of burning Guacamole retries. Set `BOREALIS_VNC_AUTH_PROBE=1` or request-scoped `auth_probe=true` only during focused diagnostics when the site-worker should perform a bounded RFB VNCAuth probe outside normal broker control; keep it off by default because each auth probe consumes an UltraVNC login attempt. Normal Remote Desktop launches go straight to Guacamole after Engine RFB banner readiness checks.
    - Site-worker Guacamole retry handles post-ready `guacd` backend status `519` by opening a fresh `guacd` session without sending additional Agent VNC lifecycle events. Guacamole VNC keeps `autoretry=3` to match the pre-hardening browser path, and the site-worker guacd startup loop is capped at 20 seconds so retry accounting stays visible and browser socket setup remains under the 30-second operator-facing connection budget. WebUI performs at most two establish attempts per Connect action and does not stack multi-minute fresh Guacamole token loops while the site-worker is still trying the same endpoint. Exhausted Guacamole backend retries close as `guacamole_unavailable`; they are not proof of bad UltraVNC credentials.
