...
- K3s baseline and operator bridge: `Engine.sh` installs and reconciles a single-node K3s control plane plus read-only `borealis-operator` workload for migration bridge work.
...
??? example "Detailed Codex Breakdown"
    ...
    - Deploy state lives in `Engine/Deploy/compose.env`, `Engine/Deploy/runtime.env`, `Engine/Deploy/webui-frontend.env`, `Engine/Deploy/image-manifest.json`, `Engine/Deploy/deploy-manifest.json`, `Engine/Deploy/k3s-baseline.sha256`, `Engine/Deploy/borealis-operator.sha256`, and `Engine/Deploy/build.log`.
    ...
    - `borealis-operator` uses `alpine:3.24` with `ca-certificates`, `tzdata`, and the shared Go api-backend binary in `borealis-operator` process mode. It runs in K3s, not Compose.
    ...
    - `borealis-operator` runs inside K3s and exposes a ClusterIP-only HMAC API on port `8088`. Stage 2 verbs are read-only cluster status commands, and its ServiceAccount is namespace-scoped without secret, node, or mutation permissions.
    ...
    - `Engine.sh --network-mode public|local deploy prod|dev`: reconciles K3s baseline and `borealis-operator` before Compose. It writes `/etc/rancher/k3s/config.yaml.d/10-borealis.yaml`, installs K3s only when the binary and `k3s.service` are missing, restarts K3s only after Borealis-owned config changes, creates the `borealis` namespace, applies namespace/node labels plus `borealis.io/k3s-config-hash` annotations, imports the operator image into K3s containerd when missing, applies fixed operator manifests, waits for rollout, and re-renders Compose env with the operator ClusterIP URL.
