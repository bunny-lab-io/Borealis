...
??? example "Detailed Codex Breakdown"
    ...
    - Raw one-line or repo-option `Engine.sh` runs sync first, then re-execs installed `Engine.sh`; local `Engine.sh --network-mode public|local deploy` uses existing on-disk source and does not update git. Sudo-origin sync validates invoking account identity, returns checkout root, `.git`, and non-runtime source ownership to that account, and prunes root-owned `Engine`, `Engine.old`, and `Agent` runtime trees from ownership reconciliation.
