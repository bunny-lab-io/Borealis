...
??? example "Detailed Codex Breakdown"
    ...
    - Site-worker Guacamole retry handles post-ready `guacd` backend status `519` by opening a fresh `guacd` session without sending additional Agent VNC lifecycle events.
