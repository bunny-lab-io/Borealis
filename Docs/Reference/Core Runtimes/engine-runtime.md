...
??? example "Detailed Codex Breakdown"
    ...
    - `job-scheduler` owns the scheduled-job tick loop, Postgres work leases, service action queueing, and `site-worker-<uuid>` reconciliation. It does not mount the host Docker socket; it asks `site-worker-orchestrator` to launch, list, inspect, stop, and remove site-worker containers and to run allowlisted Engine service actions.
    ...
    - The Engine requests the current Agent VNC password on demand over the registered Agent Socket.IO channel during `/api/vnc/establish`, uses that live credential for the Guacamole token it is minting, and does not maintain an agent-level VNC password cache. It performs TCP listener readiness checks before returning browser bootstrap data, but normal launches avoid pre-reading the RFB banner so Guacamole remains the first browser-path RFB client. Set `BOREALIS_VNC_AUTH_PROBE=1` or request-scoped `auth_probe=true` only during focused diagnostics when the site-worker should perform a bounded RFB VNCAuth probe outside normal broker control; keep it off by default because each auth probe consumes an UltraVNC login attempt. Normal Remote Desktop launches go straight to Guacamole after Engine listener readiness checks.
    - Site-worker Guacamole retry handles post-ready `guacd` backend status `519` by opening a fresh `guacd` session without sending additional Agent VNC lifecycle events. Guacamole VNC keeps `autoretry=3` to match the pre-hardening browser path, and the site-worker guacd startup loop has a bounded deadline so retry accounting stays visible and browser socket setup remains under the operator-facing connection budget. Exhausted Guacamole backend retries close as `guacamole_unavailable`; they are not proof of bad UltraVNC credentials.
    - Explicit VNCAuth diagnostic failures are reported back to the Go VNC broker as `vnc_auth_failed`. The next establish request uses Agent reason `vnc_auth_retry`, which is already supported by the Agent VNC role for runtime credential rotation and UltraVNC config rewrite.
    ...
    - `remote-desktop-guacd` uses Apache Guacamole Server 1.6.0 in VNC-only mode, binds loopback port `4822`, and mirrors guacd stdout/stderr into `Engine/Services/remote-desktop-guacd/logs/guacd.log`. It intentionally runs with upstream container defaults while the guacd hardening regression is isolated; do not treat that as permission drift.
