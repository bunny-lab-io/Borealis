...
??? example "Detailed Codex Breakdown"
    ...
    - Deploy output uses compact colored service status lines in interactive terminals; set `NO_COLOR=1` to force plain text. Shared build relationships use lineage labels such as `api-backend > job-scheduler` and `api-backend > job-scheduler > site-worker-orchestrator`.
