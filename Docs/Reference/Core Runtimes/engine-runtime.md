...
??? example "Detailed Codex Breakdown"
    ...
    - K3s readiness gates startup for PostgreSQL, API, scheduler, WireGuard, WebUI, guacd, and bridge workloads. Remaining Compose health checks gate Docker proxy, site-worker orchestrator, and Traefik ping on `127.0.0.1:8082`.
    ...
    - Server Info service rows use `borealis-operator` workload status for K3s-owned API, scheduler, PostgreSQL, WebUI, guacd, and WireGuard workloads. Docker inspect through `docker-proxy` remains only for remaining Compose bridge rows.
