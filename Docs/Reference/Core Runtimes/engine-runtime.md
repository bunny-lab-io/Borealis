...
??? example "Detailed Codex Breakdown"
    ...
    - The Engine requests the current Agent VNC password on demand over the registered Agent Socket.IO channel during `/api/vnc/establish`, uses that live credential for the Guacamole token it is minting, and does not maintain an agent-level VNC password cache. Normal launches require a non-auth RFB banner before returning browser bootstrap data, so a TCP-only or hung UltraVNC listener fails fast as `vnc_backend_no_rfb_banner` instead of burning Guacamole retries. Set `BOREALIS_VNC_AUTH_PROBE=1` or request-scoped `auth_probe=true` when the site-worker should perform a bounded RFB VNCAuth probe outside normal broker control. WebUI keeps auth probe off for first attempts, then enables request-scoped `auth_probe` on the second establish attempt only after Guacamole/browser startup fails, so Engine can classify credential failure without adding a probe to every healthy launch. Probe results are logged with structured RFB stage fields and never include the VNC password, DES challenge, or DES response.
    ...
    - `remote-desktop-guacd` uses Apache Guacamole Server 1.6.0 in VNC-only mode, binds loopback port `4822`, mirrors guacd stdout/stderr into `Engine/Services/remote-desktop-guacd/logs/guacd.log`, and captures LibVNCClient/syslog diagnostics in `Engine/Services/remote-desktop-guacd/logs/guacd-syslog.log`. It intentionally runs with upstream container defaults while the guacd hardening regression is isolated; do not treat that as permission drift.
