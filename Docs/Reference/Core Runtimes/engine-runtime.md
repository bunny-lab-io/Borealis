...
??? example "Detailed Codex Breakdown"
    ...
    - The Engine requests the current Agent VNC password on demand over the registered Agent Socket.IO channel during `/api/vnc/establish`, uses that live credential for the Guacamole token it is minting, and does not maintain an agent-level VNC password cache. It still fast-probes the advertised UltraVNC listener and waits for listener readiness before returning browser bootstrap data when that fast probe misses. Set `BOREALIS_VNC_AUTH_PROBE=1` only during focused diagnostics when the site-worker should perform a bounded RFB VNCAuth probe outside normal broker control; keep it off by default because each probe consumes an UltraVNC login attempt. The Go VNC broker requests a scoped auth probe for Remote Desktop launches so invalid UltraVNC auth fails before Guacamole opens its own retry loop.
    - Site-worker Guacamole retry handles post-ready `guacd` backend status `519` by opening a fresh `guacd` session without sending additional Agent VNC lifecycle events. Guacamole VNC `autoretry` stays disabled so Engine retry accounting remains visible and failed launches do not multiply UltraVNC connection attempts behind one browser socket.
    - Browser-side Guacamole `519` failures are reported back to the Go VNC broker as `vnc_auth_failed`. The next establish request uses Agent reason `vnc_auth_retry`, which is already supported by the Agent VNC role for runtime credential rotation and UltraVNC config rewrite.
    - VNC auth retry is single-flight per Agent. The Go broker returns `vnc_auth_retry_in_progress` or `vnc_auth_retry_settling` with `retry_after_seconds` while one credential rotation or post-rotation settle window is active, and then probes the current credential plus request-scoped RFB auth before sending another `vnc_auth_retry`. Auth-retry state clears after worker auth verification or first Guacamole frame, not after credential fetch alone. UltraVNC lockout reasons such as `too_many_auth_failures` extend the settle window instead of triggering another immediate credential rotation. Tune the settle window with `BOREALIS_VNC_AUTH_RETRY_COOLDOWN_SECONDS`.
