...
??? example "Detailed Codex Breakdown"
    ...
    - The Engine requests the current Agent VNC password on demand over the registered Agent Socket.IO channel during `/api/vnc/establish`, uses that live credential for the Guacamole token it is minting, and does not maintain an agent-level VNC password cache. It still performs a non-auth RFB banner probe against the advertised UltraVNC listener and waits for protocol readiness before returning browser bootstrap data; a TCP-open listener without an `RFB ...` banner is not treated as Guacamole-ready. Set `BOREALIS_VNC_AUTH_PROBE=1` or request-scoped `auth_probe=true` only during focused diagnostics when the site-worker should perform a bounded RFB VNCAuth probe outside normal broker control; keep it off by default because each auth probe consumes an UltraVNC login attempt. Normal Remote Desktop launches go straight to Guacamole after Engine listener readiness checks.
