...
??? example "Detailed Codex Breakdown"
    ...
    - Deploy output uses a live ANSI dashboard by default. Rows render in one table with Domain, Item, Status, and Updated columns. Domains include Frontend, Backend, Networking, Database, Reconciliation, Housekeeping, and Complete. Rows begin as `Pending...` and update in place as services build, start, and report Docker health. The renderer uses cursor-home repaint plus clear-to-end to reduce terminal flicker. Shared build relationships use lineage labels such as `api-backend > job-scheduler` and `api-backend > job-scheduler > site-worker-orchestrator`.
