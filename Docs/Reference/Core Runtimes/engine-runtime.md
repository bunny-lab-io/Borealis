...
??? example "Detailed Codex Breakdown"
    ...
    - The Engine requests the current Agent VNC password on demand over the registered Agent Socket.IO channel during `/api/vnc/establish`, uses that live credential for the Guacamole token it is minting, and does not maintain an agent-level VNC password cache. It still fast-probes the advertised UltraVNC listener and waits for listener readiness before returning browser bootstrap data when that fast probe misses. The site-worker then performs a bounded RFB VNCAuth probe before browser WebSocket handoff so stale credentials and UltraVNC lockout responses return `vnc_auth_failed` while the Go broker can still request `vnc_auth_retry`. Set `BOREALIS_VNC_AUTH_PROBE=0` only to suppress this preflight during focused emergency diagnostics.
    - Site-worker Guacamole retry asks the Agent to re-ensure `vnc_start` through the worker host-service HTTP route, not direct Socket.IO registry emit, so eventlet emits stay in the worker request context.
