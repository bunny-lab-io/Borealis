...
??? example "Detailed Codex Breakdown"
    ...
    - Deploy output uses a live ANSI dashboard by default. Image build status is grouped under Frontend Services, Backend Services, Networking Services, and Database Services; Compose status stays under Service Reconciliation; cleanup status stays under Docker Housekeeping; the final WebUI URL appears under Engine Deployment Complete. Rows begin as `Pending...` and update in place as services build, start, and report Docker health. Shared build relationships use lineage labels such as `api-backend > job-scheduler` and `api-backend > job-scheduler > site-worker-orchestrator`.
