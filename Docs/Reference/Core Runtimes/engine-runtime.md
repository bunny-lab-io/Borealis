...
??? example "Detailed Codex Breakdown"
    ...
    - `job-scheduler` uses `alpine:3.24` with Bash, Python 3, `ca-certificates`, and `tzdata`; Docker CLI, Docker Compose plugin, and Docker Buildx are not installed in this image. Stage 8 runs it as a K3s Deployment with no ServiceAccount token and no Docker socket. Legacy `site-worker-orchestrator` source still compiles, but no Stage 11 runtime service mounts `/var/run/docker.sock`, and the scheduler entrypoint fails fast if a retired orchestrator role is requested.
