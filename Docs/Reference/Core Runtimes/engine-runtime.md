...
??? example "Detailed Codex Breakdown"
    ...
    - VNC auth retry is single-flight per Agent. The Go broker returns `vnc_auth_retry_in_progress` or `vnc_auth_retry_settling` with `retry_after_seconds` while one credential rotation or post-rotation settle window is active, and then probes the current credential before sending another `vnc_auth_retry`. Tune the settle window with `BOREALIS_VNC_AUTH_RETRY_COOLDOWN_SECONDS`.
