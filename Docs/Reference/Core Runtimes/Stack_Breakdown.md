...
`Engine.sh` seeds `Engine/Services/webui-frontend/data/web-interface/` from committed WebUI source when the runtime copy is missing. It does not overwrite an existing runtime copy during normal deploys, so dev-mode Vite HMR edits survive rebuilds. The WebUI container reads that runtime copy but writes Vite optimized dependencies under a tmpfs-backed `node_modules/.vite` and Vite resolved config bundles under a tmpfs-backed `node_modules/.vite-temp`; edit files from the host, not from inside the container. Set `BOREALIS_REFRESH_WEBUI_RUNTIME_SOURCE=1` before deploy to discard and reseed the runtime WebUI source from committed source.
