...
Longhorn requires host iSCSI support for block volumes and NFSv4 client support for RWX volumes. `Engine.sh deploy` installs or verifies `open-iscsi` plus `nfs-common` on Debian-style systems, `iscsi-initiator-utils` plus `nfs-utils` on RHEL-style systems, or equivalent distro packages. It loads `iscsi_tcp`, verifies `iscsid`, and verifies `mount.nfs` before applying Longhorn. It also disables `multipathd` on dedicated Engine hosts only when no genuine multipath map exists, flushing any Longhorn-owned map by exact name. Genuine multipath storage fails closed until operator configures Longhorn device blacklist. Normal deploy does not delete Longhorn objects, volumes, PVCs, or existing PostgreSQL state.
...
- `api-backend` uses Alpine runtime image with Go API binary plus `ca-certificates`, `git`, and `tzdata`. It does not contain Python runtime or site-worker Python source. WireGuard command execution belongs to `wireguard-tunnel` through its control socket.
- `job-scheduler` uses Alpine scheduler image with compiled Go binary and without Python, Docker CLI, Docker Compose plugin, or Docker socket access. It runs only queue/reconcile mode. Retired `site-worker-orchestrator` runtime and Docker lifecycle fallback are removed, and API/scheduler image entrypoints plus Go binary reject retired orchestrator roles.
...
- `webui-frontend`, `traefik-edge`, `postgres-db`, and `remote-desktop-guacd` use service-local build contexts. `wireguard-tunnel` uses repository context so Docker can copy control server/client binaries produced by shared pinned Go build helper.
