...
`job-scheduler` is the single writer for site-worker Traefik route files. Stage 8 runs it in K3s as the authoritative scheduler manager, with `BOREALIS_SITE_WORKER_LIFECYCLE_MODE=k3s`. Empty, `auto`, unknown, and legacy `docker`, `compose`, or `site-worker-orchestrator` lifecycle modes all resolve to K3s after Compose retirement. It writes and retires `site-worker-<worker_guid>.yml` files under the dynamic Traefik config directory and uses `borealis-operator` for worker launch/list/retire plus known workload restarts. `site-worker-*` containers set `BOREALIS_SITE_WORKER_ROUTE_FILE_WRITES=0` so legacy Python route helpers keep database state only and do not write files.
