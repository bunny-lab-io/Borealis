...
`job-scheduler` is the single writer for site-worker Traefik route files. Stage 8 runs it in K3s as the authoritative scheduler manager, with `BOREALIS_SITE_WORKER_LIFECYCLE_MODE=k3s`. Empty, `auto`, or unknown lifecycle modes resolve to K3s after Compose retirement, while explicit `docker`, `compose`, and `site-worker-orchestrator` modes remain only for legacy drain tests. It writes and retires `site-worker-<worker_guid>.yml` files under the dynamic Traefik config directory and uses `borealis-operator` for worker launch/list/retire plus known workload restarts. `site-worker-*` containers set `BOREALIS_SITE_WORKER_ROUTE_FILE_WRITES=0` so legacy Python route helpers keep database state only and do not write files.
...
- `job-scheduler` uses the Alpine scheduler image without Docker CLI, Docker Compose plugin, or Docker socket access. It runs only the queue/reconcile mode. Legacy `site-worker-orchestrator` code is still compiled for migration tests, but no long-running Stage 11 service runs the Docker-boundary mode.
