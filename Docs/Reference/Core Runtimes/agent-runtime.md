...
??? example "Detailed Codex Breakdown"
    ...
    - Windows installed runtime is `C:\Borealis\Agent.exe` plus `C:\Borealis\agent.json`, managed by the native `BorealisAgent` Windows service. Fresh bootstrap from an operator-downloaded `Agent.exe` stages the runtime copy. Re-deploy with install inputs stops Borealis-managed services, scheduled tasks, and processes, preserves existing `agent.json` identity/trust state, replaces `Agent.exe`, and restarts the Agent service.
