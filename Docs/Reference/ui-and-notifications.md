...
??? example "Detailed Codex Breakdown"
    ...
    - `GET /api/server/workers` (Operator Admin Session) - returns active and recent site-worker container state, all sites with total and online device counts, connected Agent counts, and worker-assigned work for Engine Status and Sites. Sites polls this endpoint every 5 seconds and forces AG Grid to refresh the Site Worker Container renderer because the visible metric value is a nested Docker stats sample, not the stable container id.
    ...
    - Sites merges site-worker state into the main AG Grid rows. It fetches `/api/server/workers?history_seconds=300` with site load, refreshes site-worker data every 5 seconds, keeps rows ordered by site name A-Z, and shows CPU/RAM/NET/DISK mini-trends inside the Site Worker Container column, connected/disconnected/offline Agent bars, and Scheduled Jobs-style task bars. Keep the Site Worker Container column refresh value tied to the latest Docker stats/history sample so AG Grid does not skip rerendering when the container id stays unchanged.
