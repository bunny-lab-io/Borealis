...
??? example "Detailed Codex Breakdown"
    ...
    - `GET /api/server/overview` (Operator Admin Session) - returns the Server Info dashboard snapshot including service state, host runtime details, WebUI route owner/upstream, WireGuard runtime status, public-edge certificate health, and live operator presence.
    - `GET /api/server/workers` (Operator Admin Session) - returns active and recent site-worker container state, all sites with total and online device counts, connected Agent counts, and worker-assigned work for Sites and Server Info. Sites does not call this endpoint from the route loader; it shows `Polling Site Worker Metrics`, starts polling immediately after the first render, and displays mini-trends after the first successful worker payload with active Docker stats. Sites forces AG Grid to refresh the Site Worker Container renderer because the visible metric value is a nested Docker stats sample, not the stable container id.
    ...
    - Sites merges site-worker state into the main AG Grid rows. It loads site records first, starts `/api/server/workers?history_seconds=300` polling immediately after the first render, then shows CPU/RAM/NET/DISK mini-trends and Connected Devices bars after the first successful worker payload. Connected Devices renders `Analyzing Agent Connections` until that payload is ready. Connected/disconnected/offline Agent bars and Scheduled Jobs-style task bars follow the same worker payload cadence. Keep the Site Worker Container column refresh value tied to payload readiness, active worker status, and the latest Docker stats/history sample so AG Grid does not skip rerendering when the container id stays unchanged. Keep Connected Devices stable across brief zero-connected worker polls by reusing the last non-zero connected breakdown for the browser grace window; sustained zero-connected payloads must still turn the bar red.
