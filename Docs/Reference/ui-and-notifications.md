...
??? example "Detailed Codex Breakdown"
    ...
    - Sites merges site-worker state into the main AG Grid rows. It fetches `/api/server/workers?history_seconds=300` with site load, refreshes site-worker data every 5 seconds, keeps rows ordered by site name A-Z, and shows inline `Re-Deploy Site Worker` actions, Engine Status-style status pills, compact CPU/RAM/NET/DISK mini-trends, connected/disconnected/offline Agent bars, and Scheduled Jobs-style task bars.
