...
- Device Summary shows Agent Health through the readiness header role-health popover and device navigation sidebar, not as a legacy top tab. Keep agent-health composition in `Data/Engine/Containers/webui-frontend/data/web-interface/src/Devices/Tabs/Agent_Health.jsx` and `Data/Engine/Containers/webui-frontend/data/web-interface/src/Devices/Tabs/Agent_Startup_Flow.jsx`; avoid adding a separate summary-section nav item for role health.
...
- Agent Health should show startup flow like Remote Desktop readiness flows, plus clickable role/service runtime-health nodes inside the flow graph or readiness popover. Flow states are `complete`, `active`, `failed`, `pending`, and `skipped`; active nodes use the same spinner language as the linear readiness view.
