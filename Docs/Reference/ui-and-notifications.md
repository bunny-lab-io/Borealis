...
??? example "Detailed Codex Breakdown"
    ...
    - Sites merges site-worker state into the main AG Grid rows. It loads site records first, starts `/api/server/workers?history_seconds=300` polling immediately after the first render, then shows CPU/RAM/NET/DISK mini-trends and Connected Devices bars after the first successful worker payload. Connected Devices renders `Analyzing Agent Connections` until that payload is ready. Connected/disconnected/offline Agent bars and Scheduled Jobs-style task bars follow the same worker payload cadence. Keep the Site Worker Container column refresh value tied to payload readiness and the latest Docker stats/history sample so AG Grid does not skip rerendering when the container id stays unchanged. Keep Connected Devices stable across brief zero-connected worker polls by reusing the last non-zero connected breakdown for the browser grace window; sustained zero-connected payloads must still turn the bar red.
