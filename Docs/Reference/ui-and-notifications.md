...
??? example "Detailed Codex Breakdown"
    ...
    - `GET /api/server/workers` (Operator Admin Session) - returns active and recent site-worker container state, all sites with device counts, connected Agent counts, and worker-assigned work for Engine Status and Site Workers.
    ...
    - Admin Settings includes `Site Workers`, an AG Grid management page for site-worker containers and no-online placeholder site rows. It uses Borealis-blue site links, short Docker container IDs, inline `Re-Create` actions, Engine Status-style status pills, connected Agent counts, and Scheduled Jobs-style task bars.
