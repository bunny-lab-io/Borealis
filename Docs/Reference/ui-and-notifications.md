...
??? example "Detailed Codex Breakdown"
    ...
    - Search timing: debounce backend lookup until the operator has stopped typing for at least 1 second. Fetch the first 3 normalized characters as the active cache base, then filter cached rows locally as the query narrows or widens; reset the cache when search closes or drops below 3 characters.
