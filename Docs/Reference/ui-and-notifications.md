...
??? example "Detailed Codex Breakdown"
    ...
    - Sites merges site-worker state into the main AG Grid rows. It fetches `/api/server/workers?history_seconds=300` with site load, keeps rows ordered by site name A-Z, and shows inline `Re-Deploy Site Worker` actions, Engine Status-style status pills, Docker CPU/memory/Net I/O/Disk I/O stats, connected/disconnected/offline Agent bars, and Scheduled Jobs-style task bars.
    - Engine Status service and site-worker nodes use compact Docker stats chips from `/api/server/overview` and `/api/server/workers`; keep CPU, memory, Net I/O, and Disk I/O labels consistent with Sites.
