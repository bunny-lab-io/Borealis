...
??? example "Detailed Codex Breakdown"
    ...
    - Sites merges site-worker state into the main AG Grid rows. It fetches `/api/server/workers?history_seconds=300` with site load, keeps rows ordered by site name A-Z, and shows short Docker container IDs, inline `Re-Create` actions, Engine Status-style status pills, connected/disconnected/offline Agent bars, and Scheduled Jobs-style task bars.
