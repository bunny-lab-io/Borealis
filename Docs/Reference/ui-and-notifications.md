...
- Device Summary Backend Tools includes `Agent Updates` at canonical `?tab=remote_ops&view=agent_updates[&operation_id=<id>]`. Page chrome uses `Agent Updates`, System Update icon, `Update Status`, and primary `Update Now`. Dashboard uses `PageBodyFrame`, read-only React Flow progress topology above 44px Quartz history grid, expandable failures, live duration, Scheduled Job links, SSE refresh, and polling fallback.
...
??? example "Detailed Codex Breakdown"
    ...
    - `Update Now` is operator-triggered install-equivalent Agent repair. Confirmation warns about managed service restarts; action queues existing local updater through Job Scheduler and deep-links returned operation. Disable while operation remains active. When operator navigates elsewhere in Device Summary, show `Update Status` only while active operation exists.
