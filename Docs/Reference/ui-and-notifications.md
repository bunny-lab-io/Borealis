...
??? example "Detailed Codex Breakdown"
    ...
    - Admin Settings includes `Site Workers`, an AG Grid management page for site-worker containers and no-online placeholder site rows. It uses Borealis-blue site links, short Docker container IDs, inline `Re-Create` actions, Engine Status-style status pills, connected/disconnected/offline Agent bars, and Scheduled Jobs-style task bars.
