...
- Device Summary keeps an expanded `Agent Health` accordion at the top of the device navigation sidebar across every Device Details workspace. Its nested `Agent Connection` accordion consolidates API WebSocket, heartbeat, and secure-tunnel health; its nested `Roles` accordion lists remaining runtime roles. Do not restore the removed body readiness header or a legacy Agent Health top tab.
- Device Summary Backend Tools includes `Agent Updates` at canonical `?tab=remote_ops&view=agent_updates[&operation_id=<id>]`. Page chrome uses `Agent Updates`, System Update icon, and primary `Update Now`. Dashboard places two full-height glass islands directly on workspace backdrop: Remote Desktop-style progress timeline in left third and 44px Quartz history grid in remaining two thirds. Timeline retains expandable failures and live duration; history retains Scheduled Job links, SSE refresh, and polling fallback.
- Agent Health sidebar rows use neutral sidebar styling with status-specific glyphs. Row hover opens curated health details with copy control; each nested accordion also provides a master copy control. Keep composition and tooltip shaping in `Data/Engine/Containers/webui-frontend/data/web-interface/src/Devices/Tabs/Agent_Health.jsx` and `Data/Engine/Containers/webui-frontend/data/web-interface/src/Devices/Tabs/Agent_Startup_Flow.jsx`.
...
- Workspace-specific query keys are scoped to active browser. Leaving File Management clears `working_directory`, leaving Registry clears `registry_path`, and leaving Agent Updates clears `operation_id`.
...
??? example "Detailed Codex Breakdown"
    ...
    - Sites merges site-worker state into the main AG Grid rows. It loads site records first, starts `/api/server/workers?history_seconds=300` polling immediately after the first render, then shows CPU/RAM/NET/DISK mini-trends and Connected Devices bars after the first successful worker payload. Connected Devices renders `Analyzing Agent Connections` until that payload is ready. Connected/reconnecting/offline Agent bars and Scheduled Jobs-style task bars follow the same worker payload cadence. Keep the Site Worker Container column refresh value tied to payload readiness, active worker status, and the latest Docker stats/history sample so AG Grid does not skip rerendering when the container id stays unchanged. Keep Connected Devices stable across brief zero-connected worker polls by reusing the last non-zero connected breakdown for the browser grace window; sustained zero-connected payloads must still show reconnecting devices instead of implying the Agent heartbeat is offline.
    - When a site has an active site worker but no Docker stats, the Site Worker Container column uses the same muted empty-state treatment as task rows and shows `Worker Metrics Unavailable`. When a site has no active site worker, it shows `Site Worker Not Running`.
    ...
    - Sites task bars label device count separately from scheduler slot count. `Task (8 Devices)` means eight targets are represented by the row; it can be one shared Ansible work item or a grouped set of individual one-target work items.
    - Sites terminal task groups keep short-lived completed work visible while active site-worker rows continue to show current assignments.
    ...
    - `Update Now` is operator-triggered install-equivalent Agent repair. Confirmation warns about managed service restarts; action queues existing local updater through Job Scheduler and deep-links returned operation. Disable while operation remains active. Agent Updates timeline stays visible inside workspace, so page does not expose separate `Update Status` action.
    ...
    - Device List column chooser uses the same dark glass, grouped-menu treatment as Borealis context menus. Keep its group order as `Device Specs`, `Location`, `Networking`, `Heartbeat`, then `Metadata`, and sort every group A-Z by rendered label.
