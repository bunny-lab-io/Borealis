...
- Cluster conversion migrates standalone StatefulSet into CloudNativePG once, validates imported state, cuts Service traffic over, and retains old PVC plus encrypted logical dump. Supported three-node clusters use one synchronous replica acknowledgement. Writes block when durability quorum disappears, preventing stale promotion. Public membership transactions allow `1 -> 3 -> 1` only; odd membership changes and durability policy beyond three nodes remain future roadmap work.
...
??? example "Detailed Codex Breakdown"
    ...
    - `engine.cluster_onboarding_targets` records S01 operation/cluster/target identity, ordinal, approved management address/SSH port/wire key, preparation state and step, operation attempt, separate credential availability, and holder/generation/expiry for target workers. Per-operation address and fingerprint uniqueness rejects duplicate or cloned SSH targets; a partial unique address index retains uncertain targets until explicit terminal cleanup. Claim/renew/load require the sole active cluster operation, a matching target step and unexpired database-clock lease. Claims capture current controller holder plus operation attempt/step; renew/load reject controller lease loss/replacement or changed operation/attempt/step. Each new target holder advances generation. SSH workers supplement the existing membership controller; they do not own membership independently.
    - Controller lease holder is an opaque existing identity, normally hostname plus UUID or configured `BOREALIS_CLUSTER_CONTROLLER_ID`. Cohort reads must compare its exact full nonempty value. Only target-worker holder IDs use the S01 UUID contract; no schema or controller identity migration accompanies this distinction.
    - S01 worker transactions acquire shared rows in controller-lease, cluster-state, operation, Aegis-state, credential order before target access. Authority predicates are evaluated again in the following SQL statement after all locks are held. Keep parent transitions in compatible order and do not retain these locks across crypto/network calls. Target claims currently accept only the explicit inspection parent/target phase; future mutating phases require their own fixed dispatcher contract.
    - The SSH inspection queue takes a source snapshot before Aegis encryption, validates the sealed complete cohort outside the transaction, then locks cluster state and compares the snapshot again before inserting operation/targets/credentials/event/audit plus the sole active-operation reservation atomically. No partial cohort or source/Aegis mismatch may commit. Source health/membership stays unchanged. Public progress selects only explicit operation/target metadata and credential availability; typed report decoding happens after rows close. Keep ciphertext, usernames, Aegis verification tokens and raw operation payload/errors out of that projection. Admin submission/progress routes consume these contracts; full wizard integration remains unfinished.
    - SSH parent inspection transitions lock the controller lease exclusively, then cluster state and operation before checking current proof. Review locks Aegis state and all credential rows in target-ID order before targets/source members; credential cleanup uses the same credential order. Public proof encoding/SHA256 happens before the transaction, and Kubernetes observation never holds a connection. Successful inspection persists a waiting qualification boundary in existing operation payload; it does not change membership. Read-only failure/cancellation fences targets while preserving reports, returns the connection, then deletes temporary credentials. Generic retry cannot recreate SSH authority. No new schema is introduced by this lifecycle component.
    - The same unmerged S01 table definition includes `inspection_json` (typed public metadata, at most16KiB) and `inspected_at` (database completion time), plus `inspected_attempt` and `inspected_generation` (ownership that produced the report). The inspection API worker consumer requires parent kind `ssh_onboarding` and step `inspect_ssh_targets`; the controller inspection/wait boundary and atomic queue store are implemented, with Admin submission/progress routes; full wizard integration remains under implementation. Result commit repeats target/controller/operation kind/attempt/step and exact decrypted Aegis-generation fences, holding parent authority rows through a short transaction. It records `inspection_complete`, clears the target lease and appends one public event without advancing membership. Old lease replay cannot overwrite the result. Credential expiry/reset cleanup preserves public inspection evidence. Cohort reads require report ownership to match current attempt/generation and a database-clock age below five minutes; changed attempt/holder generation cannot relabel old evidence. Read every planned row before assessing eligibility so a missing credential never hides half of an expansion. [Cluster guidance](../../Engine/managing-engine-clusters.md) owns execution and field contracts.
    - Preparation scope renewal uses existing onboarding tables without schema changes. It locks controller/cluster/operation/Aegis/credential/target in existing order, compares the exact original credential ciphertext as well as its generation, and evaluates lease/credential expiry after waits. Only target lease expiry/updated time changes. Full source/cohort and unlocked-key checks surround this short renewal outside transaction scope; acquisition/consumption owns no database connection. Qualification/claim activation remains separate; [cluster guidance](../../Engine/managing-engine-clusters.md) owns the lifetime contract.
    - Network reader assembly invokes that authority/renewal pair independently for every original ordered target claim and original ciphertext. A valid anchor lease cannot authorize a sibling or rebind to its replacement credential. Each short transaction returns its connection before common-snapshot comparison, Aegis decryption or native SSH; the complete set is reread throughout consumer lifetime. This is bounded read-only authority observation, not an atomic cohort mutation/commit fence. Required PostgreSQL tests cover both claims, same-generation ciphertext replacement, revoked siblings after real controller-row waits and one-connection crypto/transport boundaries. [Original-target scope](../../Engine/managing-engine-clusters.md#original-target-network-credential-scope) owns the callback and cleanup contract.
    - `engine.cluster_onboarding_credentials` stores only Aegis-encrypted credential envelopes, generation binding and a fixed two-hour expiry. The plaintext envelope binds cluster, operation, target, address, port and approved SSH fingerprint together with one-time SSH/sudo material. No plaintext or ciphertext belongs in ordinary operation payloads/events. Shape validation, key parsing, base64 preparation and encryption finish before the queue transaction. That transaction checks the unchanged Aegis verification token under a shared row lock; readers release their connection before decrypting or contacting SSH.
    - Credential reads/renewals fail immediately after expiry, terminal operation, Aegis generation change or reset. Eligible controller startup and every30seconds reconcile cleanup with a five-second bound. Cleanup removes active secret records and fences prior worker generations while preserving target preparation/result state; credential availability is a separate field. Aegis-state deletion cascades credential deletion, including force reset and configuration restore. Tables are not in the Engine configuration export/import allowlist. Physical database backups can retain expired ciphertext; cleanup applies on restart. Start/resubmission/worker execution integration remains S01 work; the current SSH connection-check UI does not persist credentials.
    ...
    - Notes:
    - `api-backend` reads this table only for display. Service actions are queued into `job_scheduler_work_items`; `job-scheduler` claims them, routes K3s-owned workload actions through `borealis-operator`, and sends WireGuard reconcile through the mounted control socket. Stage 11 retires the Docker/Compose helper path that previously handled Traefik reload while Traefik remained Compose-owned.
