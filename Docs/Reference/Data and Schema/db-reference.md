...
- Cluster conversion migrates standalone StatefulSet into CloudNativePG once, validates imported state, cuts Service traffic over, and retains old PVC plus encrypted logical dump. Supported three-node clusters use one synchronous replica acknowledgement. Writes block when durability quorum disappears, preventing stale promotion. Five-plus durability policy remains deferred with five-plus qualification.
