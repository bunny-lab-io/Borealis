...
??? example "Detailed Codex Breakdown"
    ...
    - `POST /api/server/cluster/enable`, `/invitations`, `/admissions/{id}/approve`, and `/membership/scale` (Admin) - gated enablement and paired `1 -> 3` membership workflow. Enable accepts only private `cluster_vip` IPv4 plus exact `confirmation`; API derives current Kubernetes node name, host management IPv4, and AMD64 architecture from runtime metadata. Scale accepts `desired_size=3` only. Invitation creation/consumption and admission approval require active size one; requests that could form five-plus membership fail closed.
