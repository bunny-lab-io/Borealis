...
??? example "Detailed Codex Breakdown"
    ...
    - `GET /api/server/workers` (Admin) - active and recent `job-scheduler` site-worker state, all site names and device counts, recent assigned work, and short Docker container IDs when Docker metadata is available.
