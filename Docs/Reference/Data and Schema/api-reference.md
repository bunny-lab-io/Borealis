...
??? example "Detailed Codex Breakdown"
    ...
    - `GET /api/server/overview` (Admin) - consolidated Engine host overview used by the Server Info dashboard, including Compose-backed service state and Docker stats in container mode, public cert status, live operator sessions, WireGuard runtime state, Aegis state, and host resource basics.
    - `GET /api/server/workers` (Admin) - active and recent `job-scheduler` site-worker state, all site names plus total/online device counts, recent assigned work, and short Docker container IDs plus normalized Docker stats when Docker metadata is available.
