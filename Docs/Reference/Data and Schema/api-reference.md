...
??? example "Detailed Codex Breakdown"
    ...
    - `GET /api/patches/audit` (Token Authenticated) - list normalized patch inventory for all devices visible to the operator, including `active_install_job` when an enabled scheduled patch install already owns a matching KB, patch key, or title. Pending rows can include `patch_policy_*` source fields so Patch List can show the effective policy layer and filter policy install candidates.
    - `GET /api/patches/policies` (Token Authenticated) - list patch policies, optionally filtered by `type=global`, `type=site`, or `type=device_filter`. Rows include `eligible / raw Devices Match Policy Type` count labels plus `pending_update_count`, `pending_update_device_count`, and `pending_update_breakdown` for approved, deferral-ready install candidates owned by that effective policy row. The Windows UI combines ancestor row counts for hierarchy display.
