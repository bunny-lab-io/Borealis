...
??? example "Detailed Codex Breakdown"
    ...
    - `GET /api/server/cluster/onboarding/operations/{id}` (Admin) - recover submission receipt and read public target progress/report ownership without credential material. Exact request/response contracts and field validation live in [Managing Engine Clusters](../../Engine/managing-engine-clusters.md). Optional `qualification` contains bounded read-only check states and conditional filesystem budgets; `ready` remains false. Results grant no preparation or joining authority. See [qualification contract](../../Engine/managing-engine-clusters.md#read-only-qualification-worker-and-result-contract).
    ...
    - `GET /api/devices` (Token Authenticated) - device summary list with sparse decoded `metadata_fields`, scoped to the operator's assigned sites unless the operator is an admin.
    - `GET /api/devices/search?hostname=<query>` (Token Authenticated) - hostname search matches for the shared header search, scoped to the operator's assigned sites unless the operator is an admin. Results include `operating_system`, `agent_socket`, and `connectivity_status` for status-colored global-search OS glyphs.
    ...
    - `GET /api/repo/current_hash` (Device or Token Authenticated) - current Engine repository hash for optional `repo`, `branch`, and `ttl` query parameters; feature branch refs with slashes are supported. Agent install/update paths do not use this endpoint.
    ...
    - `GET /api/scheduled_jobs/<int:job_id>/runs` (Token Authenticated) - run history. Active SSH/WinRM readiness rows include `status = Establishing Connection` and `connection_probe_deadline_ts`.
    - `GET /api/scheduled_jobs/<int:job_id>/devices` (Token Authenticated) - device results. SSH/WinRM readiness rows expose `job_status = Establishing Connection` plus `connection_probe_deadline_ts`; scheduled patch install rows include `patch_progress` and may include `display_status_label` while keeping canonical `job_status` unchanged.
    - `DELETE /api/scheduled_jobs/<int:job_id>/runs` (Token Authenticated) - clear run history.
    - `job_kind = onboarding` on scheduled-job create/update creates an automatic local-network onboarding job. Payloads use a `device_onboarding` component and an `onboarding_scope` target. The component accepts `agent_platform` (`linux` or `windows`), `ssh_port`, `windows_port`, `winrm_port`, optional `onboarding_methods` (`smb_scm`, `scheduled_task`, `wmi_dcom`, `winrm`), and optional `onboarding_concurrency` (default `5`). The target accepts `entries` for discovery scope and optional `exclusions` for IP/FQDN/CIDR/range blacklist entries.
    ...
    - `POST /api/vnc/establish` (Token Authenticated) - establish remote desktop session for in-scope device. Optional `viewer` accepts `guacamole`. Optional `protocol` accepts `vnc` or `rdp` and defaults to `vnc`. RDP requires positive `credential_id` or manual `rdp_username`, `rdp_password`, and optional `rdp_domain`. VNC banner-readiness failures return stable error `vnc_backend_no_rfb_banner` with `probe_stage` (`dial_failed`, `banner_read_timeout`, `banner_short_read`, or `invalid_banner`), `probe_attempts`, and `probe_elapsed_ms`; raw socket errors remain Engine-log only.
    ...
    - `POST /api/server/cluster/enable`, `/invitations`, `/admissions/{id}/approve`, and `/membership/scale` (Admin) - gated enablement and paired `1 -> 3` membership workflow. Enable accepts only private `cluster_vip` IPv4; API derives current Kubernetes node name, host management IPv4, AMD64 architecture, and commit-pinned stable or `dev-<SHA-prefix>` baseline from runtime metadata. Scale accepts `desired_size=3` only. Invitation creation/consumption and admission approval require active size one; requests that could form five-plus membership fail closed.
    ...
    - `GET /api/server/cluster/releases` (Admin) - published GitHub stable and qualification release catalog with immutable SHA, `stable|qualification` channel, ancestry result, and compatibility result. Qualification requires `YYYY.MM.REVISION[.HOTFIX]-rc.N` plus GitHub prerelease status; stable tag requires normal release status.
    - `POST /api/server/cluster/hmr/start`, `/hmr/exit`, and `/updates` (Admin) - exclusive cluster-wide HMR, rolling Engine release, or distinct ordered K3s workflow. Stable Engine update accepts node or all scope with exact `UPDATE CLUSTER`. Qualification Engine update accepts all scope only with exact `DEPLOY QUALIFICATION`. K3s update accepts stable `vX.Y.Z+k3sN`, all scope only, exact `UPDATE K3S`, and one-node outage acknowledgement.
