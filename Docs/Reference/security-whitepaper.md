...
??? example "Detailed Codex Breakdown"
    ...
    - Preparation source transfer uses a fixed internal controller broker with directional AES-256-GCM keys derived from the existing operator secret, bounded request/response bodies and exact operation/target lease checks. API workers retain Kubernetes isolation and verify their actual unlocked Aegis key before/after the transfer; controllers retain sole source Job/Kubernetes ownership and never receive that key. This encrypted HTTP protocol is separate from Aegis mTLS key fanout. Detailed wire, replay, source-receipt and activation boundaries live in [Managing Engine Clusters](../Engine/managing-engine-clusters.md).
    - Normal onboarding is centrally orchestrated SSH from the existing Engine after the operator supplies target hosts and one-time Linux administration credentials. The operator installs only the first Engine. Fixed preparation/join/identity commands on additional nodes are internal remote execution steps, not an operator-local CLI workflow. S01 owns host-key approval/pinning, Aegis-scoped temporary SSH credentials, resumable provisioning and cleanup; the same central interaction model applies to future Worker Nodes under #486. Identity transfer belongs in that authenticated SSH handshake before candidate startup. Existing Aegis mTLS fanout retains its separate unlocked-data-key purpose.
    - Initial #519 implementation under `internal/engineidentity` supplies material validation and a Linux fixed-file transaction only; central SSH orchestration, admission gates and recovery integration are not wired yet. The envelope caps total input at16KiB, accepts exactly one version1 JSON object with unique exact field names, and binds canonical36-character cluster/source-node/source-UID identifiers to independently observed values. Session-secret field class is `secret`,32-256 printable ASCII bytes; it preserves punctuation. Key fields are bounded4KiB PKCS8 Ed25519 PEM and matching script public SPKI DER. There is no frontend entry or public route for this envelope. Exported bytes are secret-bearing and require private storage or authenticated confidential transport; a content digest is an internal equality guard, never source authorization.
    - The Linux transaction requires caller-held host lock and separate operation/authority and stopped-consumer checks. It snapshots only four fixed files into private mode0600 backups under a mode0700 journal, rejects symlinks/hard links/FIFOs, writes atomic replacements and syncs files/directories. Failed writes restore original bytes, ownership, modes and mtime; interrupted prepared journals restore before another attempt. If target consumers restart, rollback preserves journal and stops writing until a new stopped-consumer check passes. Committed repeats verify identity and avoid rewriting files. These library tests use temporary directories only; they are not live migration or cross-replica readiness proof.
    - `TargetBinding` requires a durable provisioning-target UUID and canonical SHA256 SSH host-key fingerprint before Kubernetes creates a Node UID. The authenticated SSH transport independently verifies the complete approved wire key; the fingerprint is a binding, not authorization. Fresh pre-staging leaves `NodeUID` empty. Joined-node recovery callers must supply and verify the real UID separately. Never substitute a provisioning UUID for a Kubernetes UID. Version2 private installation journals bind these fields exactly; any changed host, provisioning target or later lifecycle UID requires a distinct reviewed attempt and cannot rebind a prior journal. Subsequent join ownership must record the observed Kubernetes UID without rewriting the completed pre-staging journal. No version1 journal was deployed; no live compatibility conversion is introduced.
