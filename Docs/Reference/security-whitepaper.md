...
- `Engine.sh deploy` may install or verify the Longhorn host iSCSI dependency and apply the pinned Longhorn manifest, but normal deploy does not delete Longhorn volumes, PVCs, PostgreSQL state, or Aegis-protected material. Future Borealis StatefulSets should request the configured Longhorn StorageClass explicitly instead of exposing arbitrary PVC mutation through runtime services.
