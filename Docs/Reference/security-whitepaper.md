...
- K3s adds a single-node Kubernetes control plane on the Engine host. User-facing Borealis workloads move through explicit cutover stages; K3s currently hosts Longhorn storage controllers, the restricted `borealis-operator`, a non-authoritative PostgreSQL shadow StatefulSet, the authoritative API backend pod, the authoritative `job-scheduler` pod, the production WebUI ClusterIP workload, the guacd bridge workload, and operator-managed site-worker bridge pods.
- Docker socket write access is isolated to `site-worker-orchestrator`; API reads container state through a read-only Docker proxy, and K3s `job-scheduler` uses `borealis-operator` for K3s site-worker/workload lifecycle requests. It uses the authenticated orchestrator Unix socket only for legacy Docker drains and detached `Engine.sh --service` helpers while Compose-owned services remain. Other Engine services do not join the Docker group or mount the socket.
...
- Compose `remote-desktop-guacd` binds only to Engine loopback and does not mount the Docker socket. The K3s API backend binds only to Engine loopback port `5001`, owns API traffic after Stage 7, and still has no Kubernetes API or Docker socket access. The K3s guacd bridge is ClusterIP-only and is not the API target yet.
...
- `remote-desktop-guacd` runs non-root, mounts only read-only host timezone data, binds guacd to `127.0.0.1`, and does not mount the Docker socket.
...
- Stage 9 K3s PostgreSQL starts as a shadow StatefulSet with one Longhorn PVC and ClusterIP-only Services. It receives generated PostgreSQL credentials through a K3s Secret, initializes data under a PVC subdirectory, and uses a short-lived root init container with only `CHOWN`, `DAC_OVERRIDE`, and `FOWNER` to repair PVC ownership across first-run and retry paths. The PostgreSQL runtime container remains non-root with dropped capabilities, and live Engine traffic stays on Compose PostgreSQL until backup/restore import validation and explicit cutover. Shadow import validation is an explicit Engine host command that refuses to run unless the K3s StatefulSet is still marked `borealis.io/traffic-owner=docker-compose`; it resets only shadow `engine` and `assemblies` schemas before importing a logical dump from Compose PostgreSQL.
...
- The K3s `api-backend` pod has no ServiceAccount token, no kubeconfig, and no Docker socket. It receives generated runtime env through the K3s Secret `borealis-api-backend-runtime-env`, binds to `127.0.0.1:5001` by default with `hostNetwork: true`, and owns watchdog/background loops through `BOREALIS_API_BACKGROUND_LOOPS=1` after Compose API retirement. API-to-K3s-PostgreSQL validation uses a separate short-lived Job with no listener, no ServiceAccount token, no kubeconfig, and a generated Secret that overrides only `BOREALIS_DATABASE_URL` toward the K3s PostgreSQL ClusterIP Service.
- The K3s `job-scheduler` pod has no ServiceAccount token, no kubeconfig, and no Docker socket. It receives generated runtime env through the K3s Secret `borealis-job-scheduler-runtime-env`, uses `Recreate` rollout strategy to avoid duplicate scheduler loops, and temporarily uses `hostNetwork: true` only to reach the K3s API backend bridge and Compose-owned loopback PostgreSQL during bridge stages.
...
- K3s site-worker bridge pods run without ServiceAccount tokens and keep the Docker-era host-loopback route model while PostgreSQL remains localhost-only. This is a temporary Stage 5 bridge exception: pods use `hostNetwork: true`, bind remote-op ports to `127.0.0.1`, receive only fixed API logs/cache/config/secrets hostPath mounts, and still run non-root with dropped capabilities, read-only root filesystem, `RuntimeDefault` seccomp, and memory-backed `/tmp`.
...
??? example "Detailed Codex Breakdown"
    ...
    - `remote-desktop-guacd` runs non-root, binds guacd to `127.0.0.1:4822`, mounts only read-only host timezone data, writes only transient in-container file logs under tmpfs, and does not mount the Docker socket.
    ...
    - K3s bridge workers, Stage 7 API, and Stage 8 scheduler use `hostNetwork: true` and bind or call loopback only because PostgreSQL, Docker proxy, guacd, and WireGuard control paths still depend on Engine host loopback. The scheduler calls K3s API on loopback rather than a Compose API. Remove these exceptions when PostgreSQL/remote desktop cutover makes ClusterIP-only workloads practical.
    ...
    - Server Info service actions create work items. K3s `job-scheduler` sends operator-safe K3s workload actions to `borealis-operator` and asks `site-worker-orchestrator` to launch a detached helper container from the scheduler image for Compose-owned helper work.
    ...
    - Site workers still run with host networking because Traefik routes and local Engine APIs currently address per-worker loopback ports. Docker-backed workers use `site-worker-orchestrator`; K3s-backed workers use `borealis-operator` host-loopback pods. Both paths run as the `borealis-engine` runtime owner with `no-new-privileges`, dropped capabilities, read-only root filesystem, tmpfs or memory-backed `/tmp`, PID/memory/CPU caps where the runtime supports them, read-only API config/secrets, no Traefik config mount, and no `/var/run/docker.sock` mount. Only `site-worker-orchestrator` owns write access to the host Docker socket, and K3s `job-scheduler` reaches lifecycle through `borealis-operator` except for legacy drains or detached helper actions.
    - Compose-owned Server Info service actions still use a short-lived helper container with `/opt/Borealis` and the Docker socket mounted so `Engine.sh --service` can perform scoped restart, rebuild, reload, and reconcile actions. This is a documented Docker-root-equivalent exception, not a normal service boundary.
