...
- Stage 9 K3s PostgreSQL starts as a shadow StatefulSet with one Longhorn PVC and ClusterIP-only Services. It receives generated PostgreSQL credentials through a K3s Secret, but live Engine traffic stays on Compose PostgreSQL until backup/restore import validation and explicit cutover.
