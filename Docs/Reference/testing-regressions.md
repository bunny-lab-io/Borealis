...
| REG-TEST-010 | Scheduled Ansible connection admission | `TestScheduledJobAggregationShowsConnectionProbeDeadline`, `TestScheduledConnectionProbeUsesSixtySecondWindow`, and `Create_Job.connectionProbeStatus.test.jsx` | environment-gap | Jobs 380/381 skipped LAB-TRAEFIK-01 after hidden `no_handshake` readiness timeout. First deployed countdown rendered but its `(ns)` suffix was clipped by shrinkable status pills in narrow AG Grid columns. Go coverage passes; local WebUI lane lacks prepared runtime cache needed to execute countdown and no-wrap layout assertions. | Preserve persisted connection deadline, one-second UI derivation, non-wrapping status pills, 60-second SSH/WinRM window, and active-run handling. Rerun WebUI lane after Engine runtime cache preparation, then mark fixed. |
