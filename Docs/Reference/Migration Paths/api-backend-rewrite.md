...
| Current state | `M6` is complete after post-rebuild LAB-OPERATOR-01 smoke. Site-worker routes served direct `/health`, Traefik-prefixed `/health`, and prefixed Engine.IO handshakes; LAB-OPERATOR-01 registered in worker `/agents`, stayed registered past idle TTL, received a harmless generic dispatch through site-worker, recovered onto a replacement route after worker churn, and synthetic same-site socket registration/unregistration left LAB-OPERATOR-01 connected. |
| Next safe step | Start `M7: Remote Shell`. Read the existing browser shell broker and Agent remote shell role, then move shell open/send/resize/close routing to direct site-worker paths without changing remote desktop, file, process, service, or software operations. |
