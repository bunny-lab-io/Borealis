...
| Handoff Note | Local M8 implementation is in `92f93c8c` and needs post-redeploy smoke. Expected browser WebSocket path is `/_borealis/site-workers/<worker_guid>/remote-desktop/vnc/guacamole`. Check site-worker logs for `remote_desktop_session_registered` and Guacamole bridge start/end lines. If smoke fails, fix only M8 regression before marking M8 Done or starting `M9`. |
