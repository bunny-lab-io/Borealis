...
| Latest implementation commit | `9a7dea29` (`Stabilize M6 Agent socket routes`) |
| Current state | `M5` route cutover is complete. `M6` implementation is committed. Post-rebuild route-file smoke passes: synthetic queued work spawns site-workers, host Traefik route files appear, prefixed `/health` routes to workers, and prefixed `/socket.io/` returns Engine.IO handshakes. Live LAB-OPERATOR-01 smoke after Agent redeploy reached the site-worker `/agents` registry, then found two M6 regressions: idle site-workers could retire while Agent sockets were still registered, and Agents did not refresh still-usable site-worker routes by age after route churn. `9a7dea29` keeps socket-owning workers alive past idle TTL and makes Agents refresh stale site-worker routes. |
| Next safe step | Rebuild/redeploy Engine and LAB-OPERATOR-01 from branch head including `9a7dea29`, then rerun M6 Agent socket smoke: create a same-site worker route if needed, confirm the Agent connects through `/_borealis/site-workers/<worker_guid>/socket.io/`, remains registered past worker idle TTL, receives one harmless generic event, and unregisters on disconnect. |
...
| Handoff Note | Implementation commit `494f1f3d` adds a site-worker Socket.IO runtime on `/socket.io/` behind the existing `/_borealis/site-workers/<worker_guid>` Traefik prefix. Site-workers authenticate Agent bearer tokens against device GUID, fingerprint, token version, status, and site assignment; registered sockets are stored in a worker-local registry keyed by Agent ID and hostname/service mode. Generic event dispatch supports `emit` and `call` semantics for later feature migrations. Follow-up commit `f32f6c4d` mounts Traefik config into `job-scheduler` and spawned site-workers so route-file writes reach the host watched directory. Follow-up commit `d831f92a` prevents Agents from treating stale api-backend roots as valid remote-op routes and forces route refresh. Follow-up commit `9a7dea29` prevents idle site-workers from retiring while Agent sockets are registered and makes usable site-worker routes refresh by age so Agents recover from worker route churn. Post-rebuild route-file, prefixed `/health`, route lifecycle, Engine.IO handshake, and initial LAB-OPERATOR-01 worker registration smoke pass. Final post-rebuild dispatch and disconnect smoke remains pending after `9a7dea29`. |
