...
| Current state | `M5` route cutover is complete. `M6` implementation is committed. Post-rebuild route-file smoke now passes: synthetic queued work spawns site-workers, host Traefik route files appear, prefixed `/health` routes to workers, and prefixed `/socket.io/` returns Engine.IO handshakes. Existing live Agents still register on the legacy api-backend socket, so one freshly onboarded or redeployed current-branch Agent is required before `M6` can close. |
| Next safe step | Freshly onboard or redeploy one Agent from the current branch/build, then rerun M6 Agent socket smoke: create a same-site worker route if needed, confirm the Agent connects through `/_borealis/site-workers/<worker_guid>/socket.io/`, appears in worker `/agents`, receives one generic event, and unregisters on disconnect. |
...
| `M6: Site-Worker Agent Socket.IO` | `In Progress` | Move Agent remote-op event ownership to site-worker. |
...
| Handoff Note | Implementation commit `494f1f3d` adds a site-worker Socket.IO runtime on `/socket.io/` behind the existing `/_borealis/site-workers/<worker_guid>` Traefik prefix. Site-workers authenticate Agent bearer tokens against device GUID, fingerprint, token version, status, and site assignment; registered sockets are stored in a worker-local registry keyed by Agent ID and hostname/service mode. Generic event dispatch supports `emit` and `call` semantics for later feature migrations. Follow-up commit `f32f6c4d` mounts Traefik config into `job-scheduler` and spawned site-workers so route-file writes reach the host watched directory. Post-rebuild route-file, prefixed `/health`, route lifecycle, and Engine.IO handshake smoke passes. Existing live Agents still connect to api-backend, so fresh/redeployed current-branch Agent connect/disconnect and generic dispatch smoke remains pending before `M6` can be marked `Done`. |
...
- M6 post-rebuild route-file smoke passed: direct worker health, Traefik-prefixed health, and Traefik-prefixed Socket.IO polling handshake all work. Existing live Agents still register through api-backend, so fresh/redeployed current-branch Agent validation remains pending.
...
| Agent socket smoke | `M6` | `Pending` | Route-file and Engine.IO handshake smoke passed after rebuild. Remaining gate: same-site fresh/redeployed current-branch Agent should connect through the site-worker route, appear in worker registry, receive a generic event, then unregister on disconnect. |
| Agent unit tests | `M5`, `M6` | `Done` for `M5`; no Agent source changed in current `M6` implementation | `./Data/Agent/Unit_Tests/Agent_Unit_Tests.sh --domain go-agent` passed for M5 route cutover. Current M6 changes are Engine/site-worker only. |
...
- Existing live Agents still register through the api-backend Agent socket, not the site-worker socket. Freshly onboard or redeploy one Agent from the current branch/build before closing M6.
- Synthetic route-smoke work items `696` and `697` were deleted after validation.

Next work:
1. Have operator freshly onboard or redeploy one Agent from the current branch/build.
...
- Current M6 live Agent validation is still pending because existing Agents still register on api-backend socket logs after route workers are available.
