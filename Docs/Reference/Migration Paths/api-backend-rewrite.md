...
| Latest implementation commit | `4d1bb562` (`Acknowledge M11 agent maintenance dispatch`). |
| Current state | `M11` implementation and two follow-up fixes are committed. api-backend installs a generic host-service bridge that resolves the active same-site site-worker route by hostname and sends host-service `event`, `call`, and `status` traffic to that worker. WebSocket bootstrap keeps legacy api-backend socket helpers under `legacy_*` names but no longer overwrites the worker bridge. Quick-job dispatch paths now use the worker bridge when the Engine runtime is registered; update-agent and watchdog quick-job/service remediation no longer fall back to the legacy api-backend Agent socket registry. Commit `3c1978a9` queues allowlisted one-way live events inside replacement site-workers during Agent socket handoff. Commit `4d1bb562` makes Agent maintenance dispatch call the registered SYSTEM socket for an Agent acknowledgement, records Agent errors/no-response as failed job history, and keeps queueing only for absent sockets during handoff. Operator smoke confirmed the 7-Zip install quick job succeeded after `3c1978a9`; Agent update smoke failed before `4d1bb562` because `agent.json` still had an empty `update` object. Post-redeploy Agent maintenance smoke is pending again. |
| Next safe step | Rebuild/redeploy Engine, then smoke `M11` in the WebUI against `LAB-OPERATOR-01`: run one Agent maintenance update request and confirm the job history/activity shows dispatch acknowledgement, `agent.json` gets an `agent.update.operation_id`, and the operation reaches a terminal result after heartbeat. Re-run one normal quick job only if operator wants final full M11 confirmation. No Agent redeploy is required for commit `4d1bb562`. Do not start `M12` until M11 quick-job and Agent maintenance smoke passes and this tracker marks `M11` Done. |
...
- M11 implementation, handoff fix, and Agent acknowledgement fix committed: live quick-job and Agent maintenance dispatch now route through site-workers, selected one-way events queue during replacement-worker Agent socket handoff, and Agent maintenance requires a live Agent acknowledgement when the SYSTEM socket is registered. Post-redeploy Agent maintenance smoke remains pending.
...
- Complete `M11` post-redeploy smoke, then mark `M11` Done before starting `M12`.
...
We are on branch feature/rewrite-api-backend-in-golang for PR #232, "Rewrite api-backend in Golang". Branch head should include M11 implementation commit `2b43e27e`, M11 handoff fix commit `3c1978a9`, and tracker updates showing M11 post-redeploy smoke is pending.

M1 through M10 are Done. M11 is active.
...
3. If M11 smoke passes, update this tracker: mark M11 Done, add validation row, set next safe step to M12.
4. If M11 smoke fails, fix only M11 regression, validate, commit, push, and retry smoke.
5. Do not start `M12` until M11 smoke passes and this tracker marks M11 `Done`.
