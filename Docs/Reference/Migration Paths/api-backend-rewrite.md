...
| Latest implementation commit | `46cc9833` (`M8 Remote Desktop worker Guacamole route`). |
...
| Next safe step | Operator rebuild/redeploy Engine from branch head at or after `46cc9833`, then run M8 smoke: open Remote Desktop for LAB-OPERATOR-01, verify browser WebSocket uses `/_borealis/site-workers/<worker_guid>/remote-desktop/vnc/guacamole`, verify the desktop renders and disconnect cleans up worker-local session state. Do not start `M9` until post-redeploy M8 smoke passes and this tracker marks `M8` Done. |
...
| Handoff Note | Local M8 implementation is in `46cc9833` and needs post-redeploy smoke. Expected browser WebSocket path is `/_borealis/site-workers/<worker_guid>/remote-desktop/vnc/guacamole`. Check site-worker logs for `remote_desktop_session_registered` and Guacamole bridge start/end lines. If smoke fails, fix only M8 regression before marking M8 Done or starting `M9`. |
