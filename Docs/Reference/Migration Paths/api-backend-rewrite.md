...
| Latest implementation commit | This `M11` stale Agent socket retry fix change set. Previous pushed implementation commit is `860d4319` (`Defer agent update config write until after ack`). |
| Current state | `M12` is complete: operator smoke confirmed manual Ansible Quick Job `112` and Scheduled Job `113` succeeded on `LAB-DOCS-01` through site-worker Ansible dispatch. `M11` remains active. Post-rebuild Agent maintenance smoke against `LAB-OPERATOR-01` produced operation id `e7bddea1-9351-40eb-bc21-1bc4ea4ed855` and still failed with Agent acknowledgement timeout. Runtime DB confirms `LAB-OPERATOR-01` is running build `860d4319f45f8d8d0c0d23c8e0299be64f68f407`, so this is not stale Agent binary. Site-worker log `site-workers/98fba812-e925-49fb-a8f0-032193655499.log` shows the SYSTEM socket registered, then disconnected during the maintenance call window before re-registering after the job had already failed. The local Engine fix treats `agent_maintenance_request` no-response from a registered socket as a stale SID/reconnect case and queues the same allowlisted maintenance event for the next Agent registration instead of failing the run immediately. |
| Next safe step | Commit/push the stale Agent socket retry fix, redeploy the Engine so site-workers pick it up, then smoke `M11` again: run one Agent maintenance update, confirm job history/activity shows `queued` or `delivered`, confirm `agent.json` gets `agent.update.operation_id`, and confirm heartbeat reconciles the operation to a terminal job result. Agent rebuild is not required for this Engine-only fix. |
