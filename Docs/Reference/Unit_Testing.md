...
??? example "Detailed Codex Breakdown"
    ...
    - Every database-backed Go integration test in the API package reads `BOREALIS_TEST_DATABASE_URL` directly or through a package-local test helper. The Go syntax walker follows helper references and compares discovered test names with the maintained inventory; register new tests in the same PR.
