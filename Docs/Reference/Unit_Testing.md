...
??? example "Detailed Codex Breakdown"
    ...
    - Source storage tests in `cluster_ssh_storage_test.go` and `cluster_ssh_storage_scope_test.go` cover expansion/replacement CNPG layouts, exact PVC/PV/Longhorn identity and size binding, actual replica policy, retained detached occupation, owner/placement errors, malformed quantities/JSON and unsupported storage settings. Full scope tests reject initial/final inventory drift, source/target authority drift, ignored failures, independent-context cancellation and escaped readers, while permitting list reordering and collection-clock changes. An actual TLS listener exercises the fixed controller adapter, request allowlist, redirect refusal, response bounds and private errors. Run `go -C Data/Engine/Containers/api-backend test ./cmd/api-backend -run '^TestClusterSSHStorage' -count=1`, then full Engine Go. These fixtures add no PostgreSQL inventory entry or dependency and do not qualify target disk capacity, provisioning classes or production storage.
    - `TestClusterSSHStorageBroker*` composes the controller TLS storage adapter, fresh source Jobs/Secret reader, encrypted response version3 and worker validation. Expansion/replacement cases require three complete storage inventories and two source Job rounds per read; drift during Jobs/Secret reads, changed receipts on reread, cancellation, malformed projections and caller aliasing fail closed. `TestClusterSSHSourceBroker*` retains replay/envelope/legacy-version/error coverage. Existing required `TestClusterSSHSourceBrokerPostgresAuthorityAndPrivateTransfer` now includes storage observations, drift during source work and changed storage across requests, with a one-connection pool proving no connection survives Kubernetes work. Run `go -C Data/Engine/Containers/api-backend test ./cmd/api-backend -run '^TestClusterSSH(Storage|SourceBroker|PreparationSource)' -count=1`, then full Engine Go and required PostgreSQL lane; inventory stays41tests and rejects skips. No live activation, target capacity proof or new permission follows from these tests.
