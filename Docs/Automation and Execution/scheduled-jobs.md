...
- `Agent.exe` reports the remote hostname through onboarding state and events. The scheduler stores that value in `scheduled_job_onboarding_targets.target_hostname`, while preserving `target_address`, so the Onboarding Summary can render `address:port (hostname)` when the hostname is known. Machine markers remain in `bootstrap.log` for diagnostics but are not shown in operator-facing onboarding output.
