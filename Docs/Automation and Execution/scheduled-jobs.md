...
- Linux targets use SSH. At run time the Engine probes TCP/SSH, authenticates with the selected credential, confirms the target is Linux, downloads `Data/Agent/dist/linux-amd64/Agent.exe` from the selected branch, and runs `Agent.exe --server-url <engine_url> --site-enrollment-code <site_code> --install-service`.
