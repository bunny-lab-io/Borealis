...
??? example "Detailed Codex Breakdown"
    ...
    - The Agent downloads the matched update when needed, uses WUA async `BeginDownload`/`EndDownload` and `BeginInstall`/`EndInstall`, posts live progress to `/api/agent/patches/install-progress`, never reboots the device, captures available stdout/stderr/result data, and refreshes patch inventory after the attempt.
