...
Server Info can queue supported Engine service actions through the job scheduler. K3s-owned workload actions are routed through `borealis-operator`; the remaining Docker/Compose helper action is Traefik reload through `site-worker-orchestrator`, which only accepts allowlisted actions.
...
??? example "Detailed Codex Breakdown"
    ...
    - Service actions queue work items so API request can return before service changes interrupt runtime. K3s-owned workload actions are reconciled through `borealis-operator`; K3s WireGuard reconcile uses the mounted WireGuard control socket from `job-scheduler`; remaining Docker/Compose helper action is Traefik reload through `site-worker-orchestrator`. Server Info shows `site-worker-orchestrator` but does not expose an operator restart action for it.
