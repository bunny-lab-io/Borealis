...
- Remote Desktop focuses the Guacamole display surface after attach, ready, display resize, and Ctrl+Alt+Del actions so keyboard input lands in the remote desktop without requiring a manual reconnect.
- If the Guacamole browser stream closes after it was ready, the WebUI silently re-establishes the viewer up to three times. This covers Windows secure-desktop to user-desktop transitions without forcing the operator to disconnect and reconnect manually.
- The Session Control speed/quality slider defaults to the far-left speed preference for new operator browser state.
- Agent-side VNC credential requests use a fast path when the VNC role already reports running/listening with an active firewall scope; full service/config reconciliation remains reserved for stale, rotated, or not-ready credential paths.
