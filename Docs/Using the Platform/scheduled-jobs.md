...
??? example "Detailed Codex Breakdown"
    ...
    - `job-scheduler` owns scheduled ticks, queue leases, service-action queueing, and K3s site-worker reconciliation. K3s workload/site-worker lifecycle goes through `borealis-operator`; remaining Docker/Compose helper execution goes through `site-worker-orchestrator`.
