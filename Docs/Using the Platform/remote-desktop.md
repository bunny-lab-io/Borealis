...
??? example "Detailed Codex Breakdown"
    ...
    - When live Agent topology only reports the primary display but the Guacamole framebuffer is clearly wider than that monitor, WebUI keeps reported `Display 1` as the primary region and infers the extra horizontal framebuffer space as `Display 2`, preferring a left-side secondary display when the primary display is the reported monitor.
