...
??? example "Detailed Codex Breakdown"
    ...
    - Site-worker registration performs a bounded RFB VNCAuth probe by default before the browser socket opens. This catches stale UltraVNC credentials and lockout responses early enough for the Go broker to request `vnc_auth_retry`, rotate the Agent credential, restart UltraVNC, and issue a fresh Guacamole token. Set `BOREALIS_VNC_AUTH_PROBE=0` only for focused emergency troubleshooting where the extra preflight VNC login must be suppressed.
    ...
    - Guacamole startup treats post-ready backend status `519` as retryable, asks the Agent to re-ensure `vnc_start` once with the same session and firewall scope, opens a fresh `guacd` session, and keeps `participant_id` plus token hints in VNC proxy logs for browser-to-Engine correlation. Guacd-side retries are intentionally bounded so stale credentials do not trip UltraVNC's failed-login lockout before the broker can rotate credentials.
    ...
    - Agent VNC readiness serializes local service checks, gives pending services time to settle, force-kills stuck `STOP_PENDING` UltraVNC processes before start and after failed start verification, and waits for the listener before reporting ready.
