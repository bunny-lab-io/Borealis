...
??? example "Detailed Codex Breakdown"
    ...
    - Remote Desktop speed/quality preferences flow from WebUI through the Go VNC broker into the site-worker Guacamole session as `-2` through `2`; the default favors speed for lower-end endpoints. Speed-biased sessions advertise JPEG/WebP before PNG during Guacamole image negotiation so large ultrawide and multi-monitor framebuffers avoid lossless-heavy update bursts that can appear as black flicker during transport pressure.
