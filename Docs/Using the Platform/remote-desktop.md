...
??? example "Detailed Codex Breakdown"
    ...
    - Guacamole startup treats post-ready backend status `519` as retryable, asks the Agent to re-ensure `vnc_start` once with the same session and firewall scope, opens a fresh `guacd` session, and keeps `participant_id` plus token hints in VNC proxy logs for browser-to-Engine correlation.
