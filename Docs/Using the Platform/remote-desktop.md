...
??? example "Detailed Codex Breakdown"
    ...
    - RDP role owns only firewall rule `Borealis - RDP - WireGuard`. Rule allows TCP 3389 from Engine WireGuard `/32` to Agent WireGuard `/32`; ensure loop replaces this exact rule only when its managed scope drifts and leaves unrelated Windows Firewall configuration untouched.
