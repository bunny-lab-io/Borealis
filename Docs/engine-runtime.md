...
- Remote desktop proxy: `Data/Engine/services/RemoteDesktop/` (noVNC WebSocket bridge plus Apache Guacamole VNC bridge).
...
- noVNC remains the default viewer. Apache Guacamole VNC uses local `guacd` on `127.0.0.1:4822` by default, is served through `/remote-desktop/vnc/guacamole`, and never returns the UltraVNC password to the browser.
