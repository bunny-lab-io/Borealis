...
| `./Tests/run-engine-go.sh` | Go API/scheduler/operator format, tidy drift, vet, tests, binary, route/cutover policy | Go 1.25.12 |
| `./Tests/run-engine-python.sh [--domain NAME]` | Inventory-backed retained Python suites in isolated files | Python 3.12 and venv |
...
| Engine / go-api-scheduler-operator | Go API and role inputs | `./Tests/run-engine-go.sh` | Go format/tidy/vet/test/build, routes, cutover | Public API, scheduler, operator regressions | under 10 min |
| Engine / retained-python | Python worker/tests | `./Tests/run-engine-python.sh` | Every inventoried test file in isolation | Missing or hidden retained suite | under 20 min |
...
- All 19 retained Engine Python files recorded across nine explicit domains.
...
- Cutover policy prevents return of retired Compose workloads, Python public API/WebUI ownership, public Python listeners, legacy orchestrator role, and direct kubectl lifecycle bypass.
...
- Replaced 20 stale Engine test mappings with complete machine-readable ownership, including `test_metadata_fields.py`.
